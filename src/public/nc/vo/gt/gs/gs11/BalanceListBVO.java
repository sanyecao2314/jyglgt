/***************************************************************\
 *     The skeleton of this class is generated by an automatic *
 * code generator for NC product. It is based on Velocity.     *
\***************************************************************/
package nc.vo.gt.gs.gs11;
	
import nc.vo.pub.*;
import nc.vo.pub.lang.*;
	
/**
 * <b> 在此处简要描述此类的功能 </b>
 * <p>
 *     在此处添加此类的描述信息
 * </p>
 * 创建日期:2011-06-23 15:23:29
 * @author 施坤
 * @version NCPrj 1.0
 */
@SuppressWarnings("serial")
public class BalanceListBVO extends SuperVO {
	private String pk_corp;
	private String deliveryno;
	private UFDateTime ts;
	private UFDouble vdef9;
	private String vfree2;
	private UFDouble vdef10;
	private String pk_sellpaymentjs;
	private UFDouble hsprice;
	private String vdef15;
	private String pk_cargdoc;
	private String vdef14;
	private UFDouble endbalancemny;
	private UFDouble vdef7;
	private UFDouble preferential;
	private String disvehicleno;
	private String pk_cubasdoc;
	private String pricebz;
	private String vdef16;
	private UFDouble vdef2;
	private UFDate dates;
	private UFDouble vdef5;
	private UFDouble weight;
	private String vdef19;
	private String pk_invbasdoc;
	private String memo;
	private String vdef18;
	private UFDouble vdef4;
	private String balancetype;
	private String times;
	private String vdef17;
	private UFDouble vdef1;
	private String vdef20;
	private UFDouble vdef8;
	private String vfree4;
	private String sellinfocode;
	private UFDouble balanceprice;
	private String vfree5;
	private String gatheringcode;
	private String vdef13;
	private String vfree3;
	private String pk_salescontract;
	private String poundcode;
	private String vfree1;
	private String vdef11;
	private UFDouble num;
	private UFDouble balancemny;
	private String vdef12;
	private UFDouble vdef3;
	private UFDouble vdef6;
	private Integer dr;
	private String pk_invmandoc;
	private String pk_balancelist_b;
	private String pk_pricepolicy_b;
	private String pricepolicycode;
	private String pk_productline;
	private UFDouble cdprice;
	private UFDouble montax;
	private String priceway;
	private String pk_custproduct;
	private UFDouble twonum; 
	private String vfree6;
	private String vfree7;
	private String vfree8;
	private String vfree9;
	private String vfree10;
	private String pk_measure_b ; // 销售发货磅单子表主键
	private String balanceflag;   //结算标识
	private String issuecode;     //出库单号
	private String pk_stordoc;    //仓库主键
	private String pk_saleorder_b;//销售订单子表主键
	private String pk_cumandoc;   //客商档案主键
	private String pk_upper_b;    //上游子表主键
	private String pk_balancelisttemp;//结算临时表主键
	private UFDouble preferentialmny; //优惠金额
	private UFBoolean fyhflag;        //是否负优惠
	private String pk_receiveway;     //收款方式
	private String vreceiveway;      //收款方式名称(传输数据，数据库无此字段)
	private String vinvcode;      //存货编码(传输数据，数据库无此字段)
	private String vinvname;      //存货名称(传输数据，数据库无此字段)
	private String vinvspec;      //存货规格(传输数据，数据库无此字段)
	private String vvlevel; //等级名称(传输数据，数据库无此字段) 
		
	public static final String PK_CORP = "pk_corp";
	public static final String DELIVERYNO = "deliveryno";
	public static final String VDEF9 = "vdef9";
	public static final String VFREE2 = "vfree2";
	public static final String VDEF10 = "vdef10";
	public static final String PK_SELLPAYMENTJS = "pk_sellpaymentjs";
	public static final String HSPRICE = "hsprice";
	public static final String VDEF15 = "vdef15";
	public static final String PK_CARGDOC = "pk_cargdoc";
	public static final String VDEF14 = "vdef14";
	public static final String ENDBALANCEMNY = "endbalancemny";
	public static final String VDEF7 = "vdef7";
	public static final String PREFERENTIAL = "preferential";
	public static final String DISVEHICLENO = "disvehicleno";
	public static final String PK_CUBASDOC = "pk_cubasdoc";
	public static final String PRICEBZ = "pricebz";
	public static final String VDEF16 = "vdef16";
	public static final String VDEF2 = "vdef2";
	public static final String DATES = "dates";
	public static final String VDEF5 = "vdef5";
	public static final String WEIGHT = "weight";
	public static final String VDEF19 = "vdef19";
	public static final String PK_INVBASDOC = "pk_invbasdoc";
	public static final String MEMO = "memo";
	public static final String VDEF18 = "vdef18";
	public static final String VDEF4 = "vdef4";
	public static final String BALANCETYPE = "balancetype";
	public static final String TIMES = "times";
	public static final String VDEF17 = "vdef17";
	public static final String VDEF1 = "vdef1";
	public static final String VDEF20 = "vdef20";
	public static final String VDEF8 = "vdef8";
	public static final String VFREE4 = "vfree4";
	public static final String SELLINFOCODE = "sellinfocode";
	public static final String BALANCEPRICE = "balanceprice";
	public static final String VFREE5 = "vfree5";
	public static final String VFREE6 = "vfree6";
	public static final String VFREE7 = "vfree7";
	public static final String VFREE8 = "vfree8";
	public static final String VFREE9 = "vfree9";
	public static final String VFREE10 = "vfree10";
	public static final String GATHERINGCODE = "gatheringcode";
	public static final String VDEF13 = "vdef13";
	public static final String VFREE3 = "vfree3";
	public static final String PK_SALESCONTRACT = "pk_salescontract";
	public static final String POUNDCODE = "poundcode";
	public static final String VFREE1 = "vfree1";
	public static final String VDEF11 = "vdef11";
	public static final String NUM = "num";
	public static final String BALANCEMNY = "balancemny";
	public static final String VDEF12 = "vdef12";
	public static final String VDEF3 = "vdef3";
	public static final String VDEF6 = "vdef6";
	public static final String PK_INVMANDOC = "pk_invmandoc";
	public static final String PK_BALANCELIST_B = "pk_balancelist_b";
	public static final String PK_MEASURE_B = "pk_measure_b";
	public static final String BALANCEFLAG = "balanceflag";
	public static final String ISSUECODE = "issuecode";
	public static final String PK_STORDOC = "pk_stordoc";
	public static final String PK_SALEORDER_B = "pk_saleorder_b";
	public static final String PK_CUMANDOC = "pk_cumandoc";
	public static final String PK_UPPER_B = "pk_upper_b";
	public static final String PK_BALANCELISTTEMP = "pk_balancelisttemp";
	public static final String PREFERENTIALMNY="preferentialmny";
	public static final String FYHFLAG="fyhflag";
	public static final String PK_RECEIVEWAY="pk_receiveway";
	public static final String VRECEIVEWAY="vreceiveway";
	public static final String PK_PRICEPOLICY_B="pk_pricepolicy_b";
	public static final String PRICEPOLICYCODE="pricepolicycode";
	public static final String PK_PRODUCTLINE="pk_productline";
	public static final String CDPRICE="cdprice";
	public static final String MONTAX="montax";
	public static final String PRICEWAY="priceway";
	public static final String MONTXMNY="montxmny";
	public static final String PK_CUSTPRODUCT="pk_custproduct";
	public static final String TWONUM="twonum";
	

	
	public String getVfree6() {
		return vfree6;
	}
	public void setVfree6(String vfree6) {
		this.vfree6 = vfree6;
	}
	public String getVfree7() {
		return vfree7;
	}
	public void setVfree7(String vfree7) {
		this.vfree7 = vfree7;
	}
	public String getVfree8() {
		return vfree8;
	}
	public void setVfree8(String vfree8) {
		this.vfree8 = vfree8;
	}
	public String getVfree9() {
		return vfree9;
	}
	public void setVfree9(String vfree9) {
		this.vfree9 = vfree9;
	}
	public String getVfree10() {
		return vfree10;
	}
	public void setVfree10(String vfree10) {
		this.vfree10 = vfree10;
	}
	public UFDouble getTwonum() {
		return twonum;
	}
	public void setTwonum(UFDouble twonum) {
		this.twonum = twonum;
	}
	public String getPk_custproduct() {
		return pk_custproduct;
	}
	public void setPk_custproduct(String pk_custproduct) {
		this.pk_custproduct = pk_custproduct;
	}
	public String getPk_productline() {
		return pk_productline;
	}
	public void setPk_productline(String pk_productline) {
		this.pk_productline = pk_productline;
	}
	public UFDouble getCdprice() {
		return cdprice;
	}
	public void setCdprice(UFDouble cdprice) {
		this.cdprice = cdprice;
	}
	public UFDouble getMontax() {
		return montax;
	}
	public void setMontax(UFDouble montax) {
		this.montax = montax;
	}
	public String getPriceway() {
		return priceway;
	}
	public void setPriceway(String priceway) {
		this.priceway = priceway;
	}

	public String getPricepolicycode() {
		return pricepolicycode;
	}
	public void setPricepolicycode(String pricepolicycode) {
		this.pricepolicycode = pricepolicycode;
	}
	public String getPk_pricepolicy_b() {
		return pk_pricepolicy_b;
	}
	public void setPk_pricepolicy_b(String pk_pricepolicy_b) {
		this.pk_pricepolicy_b = pk_pricepolicy_b;
	}
	public String getVinvcode() {
		return vinvcode;
	}
	public void setVinvcode(String vinvcode) {
		this.vinvcode = vinvcode;
	}
	public String getVinvname() {
		return vinvname;
	}
	public void setVinvname(String vinvname) {
		this.vinvname = vinvname;
	}
	public String getVinvspec() {
		return vinvspec;
	}
	public void setVinvspec(String vinvspec) {
		this.vinvspec = vinvspec;
	}
	public String getVvlevel() {
		return vvlevel;
	}
	public void setVvlevel(String vvlevel) {
		this.vvlevel = vvlevel;
	}
	public String getPk_receiveway() {
		return pk_receiveway;
	}
	public void setPk_receiveway(String pk_receiveway) {
		this.pk_receiveway = pk_receiveway;
	}
	public UFBoolean getFyhflag() {
		return fyhflag;
	}
	public void setFyhflag(UFBoolean fyhflag) {
		this.fyhflag = fyhflag;
	}
	public UFDouble getPreferentialmny() {
		return preferentialmny;
	}
	public void setPreferentialmny(UFDouble preferentialmny) {
		this.preferentialmny = preferentialmny;
	}
	public String getPk_balancelisttemp() {
		return pk_balancelisttemp;
	}
	public void setPk_balancelisttemp(String pk_balancelisttemp) {
		this.pk_balancelisttemp = pk_balancelisttemp;
	}
	public String getPk_upper_b() {
		return pk_upper_b;
	}
	public void setPk_upper_b(String pk_upper_b) {
		this.pk_upper_b = pk_upper_b;
	}
	public String getPk_cumandoc() {
		return pk_cumandoc;
	}
	public void setPk_cumandoc(String pk_cumandoc) {
		this.pk_cumandoc = pk_cumandoc;
	}
	public String getPk_saleorder_b() {
		return pk_saleorder_b;
	}
	public void setPk_saleorder_b(String pk_saleorder_b) {
		this.pk_saleorder_b = pk_saleorder_b;
	}
	public String getPk_stordoc() {
		return pk_stordoc;
	}
	public void setPk_stordoc(String pk_stordoc) {
		this.pk_stordoc = pk_stordoc;
	}
	public String getIssuecode() {
		return issuecode;
	}
	public void setIssuecode(String issuecode) {
		this.issuecode = issuecode;
	}
	public String getBalanceflag() {
		return balanceflag;
	}
	public void setBalanceflag(String balanceflag) {
		this.balanceflag = balanceflag;
	}
	public String getPk_measure_b() {
		return pk_measure_b;
	}
	public void setPk_measure_b(String pk_measure_b) {
		this.pk_measure_b = pk_measure_b;
	}
	/**
	 * 属性pk_corp的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return String
	 */
	public String getPk_corp () {
		return pk_corp;
	}   
	/**
	 * 属性pk_corp的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newPk_corp String
	 */
	public void setPk_corp (String newPk_corp ) {
	 	this.pk_corp = newPk_corp;
	} 	  
	/**
	 * 属性deliveryno的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return String
	 */
	public String getDeliveryno () {
		return deliveryno;
	}   
	
	public String getVreceiveway() {
		return vreceiveway;
	}
	public void setVreceiveway(String vreceiveway) {
		this.vreceiveway = vreceiveway;
	}
	/**
	 * 属性deliveryno的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newDeliveryno String
	 */
	public void setDeliveryno (String newDeliveryno ) {
	 	this.deliveryno = newDeliveryno;
	} 	  
	/**
	 * 属性ts的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return UFDateTime
	 */
	public UFDateTime getTs () {
		return ts;
	}   
	/**
	 * 属性ts的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newTs UFDateTime
	 */
	public void setTs (UFDateTime newTs ) {
	 	this.ts = newTs;
	} 	  
	/**
	 * 属性vdef9的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return UFDouble
	 */
	public UFDouble getVdef9 () {
		return vdef9;
	}   
	/**
	 * 属性vdef9的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newVdef9 UFDouble
	 */
	public void setVdef9 (UFDouble newVdef9 ) {
	 	this.vdef9 = newVdef9;
	} 	  
	/**
	 * 属性vfree2的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return String
	 */
	public String getVfree2 () {
		return vfree2;
	}   
	/**
	 * 属性vfree2的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newVfree2 String
	 */
	public void setVfree2 (String newVfree2 ) {
	 	this.vfree2 = newVfree2;
	} 	  
	/**
	 * 属性vdef10的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return UFDouble
	 */
	public UFDouble getVdef10 () {
		return vdef10;
	}   
	/**
	 * 属性vdef10的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newVdef10 UFDouble
	 */
	public void setVdef10 (UFDouble newVdef10 ) {
	 	this.vdef10 = newVdef10;
	} 	  
	/**
	 * 属性pk_sellpaymentjs的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return String
	 */
	public String getPk_sellpaymentjs () {
		return pk_sellpaymentjs;
	}   
	/**
	 * 属性pk_sellpaymentjs的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newPk_sellpaymentjs String
	 */
	public void setPk_sellpaymentjs (String newPk_sellpaymentjs ) {
	 	this.pk_sellpaymentjs = newPk_sellpaymentjs;
	} 	  
	/**
	 * 属性hsprice的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return UFDouble
	 */
	public UFDouble getHsprice () {
		return hsprice;
	}   
	/**
	 * 属性hsprice的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newHsprice UFDouble
	 */
	public void setHsprice (UFDouble newHsprice ) {
	 	this.hsprice = newHsprice;
	} 	  
	/**
	 * 属性vdef15的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return String
	 */
	public String getVdef15 () {
		return vdef15;
	}   
	/**
	 * 属性vdef15的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newVdef15 String
	 */
	public void setVdef15 (String newVdef15 ) {
	 	this.vdef15 = newVdef15;
	} 	  
	/**
	 * 属性pk_cargdoc的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return String
	 */
	public String getPk_cargdoc () {
		return pk_cargdoc;
	}   
	/**
	 * 属性pk_cargdoc的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newPk_cargdoc String
	 */
	public void setPk_cargdoc (String newPk_cargdoc ) {
	 	this.pk_cargdoc = newPk_cargdoc;
	} 	  
	/**
	 * 属性vdef14的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return String
	 */
	public String getVdef14 () {
		return vdef14;
	}   
	/**
	 * 属性vdef14的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newVdef14 String
	 */
	public void setVdef14 (String newVdef14 ) {
	 	this.vdef14 = newVdef14;
	} 	  
	/**
	 * 属性endbalancemny的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return UFDouble
	 */
	public UFDouble getEndbalancemny () {
		return endbalancemny;
	}   
	/**
	 * 属性endbalancemny的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newEndbalancemny UFDouble
	 */
	public void setEndbalancemny (UFDouble newEndbalancemny ) {
	 	this.endbalancemny = newEndbalancemny;
	} 	  
	/**
	 * 属性vdef7的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return UFDouble
	 */
	public UFDouble getVdef7 () {
		return vdef7;
	}   
	/**
	 * 属性vdef7的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newVdef7 UFDouble
	 */
	public void setVdef7 (UFDouble newVdef7 ) {
	 	this.vdef7 = newVdef7;
	} 	  
	/**
	 * 属性preferential的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return UFDouble
	 */
	public UFDouble getPreferential () {
		return preferential;
	}   
	/**
	 * 属性preferential的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newPreferential UFDouble
	 */
	public void setPreferential (UFDouble newPreferential ) {
	 	this.preferential = newPreferential;
	} 	  
	/**
	 * 属性disvehicleno的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return String
	 */
	public String getDisvehicleno () {
		return disvehicleno;
	}   
	/**
	 * 属性disvehicleno的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newDisvehicleno String
	 */
	public void setDisvehicleno (String newDisvehicleno ) {
	 	this.disvehicleno = newDisvehicleno;
	} 	  
	/**
	 * 属性pk_cubasdoc的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return String
	 */
	public String getPk_cubasdoc () {
		return pk_cubasdoc;
	}   
	/**
	 * 属性pk_cubasdoc的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newPk_cubasdoc String
	 */
	public void setPk_cubasdoc (String newPk_cubasdoc ) {
	 	this.pk_cubasdoc = newPk_cubasdoc;
	} 	  
	/**
	 * 属性pricebz的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return String
	 */
	public String getPricebz () {
		return pricebz;
	}   
	/**
	 * 属性pricebz的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newPricebz String
	 */
	public void setPricebz (String newPricebz ) {
	 	this.pricebz = newPricebz;
	} 	  
	/**
	 * 属性vdef16的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return String
	 */
	public String getVdef16 () {
		return vdef16;
	}   
	/**
	 * 属性vdef16的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newVdef16 String
	 */
	public void setVdef16 (String newVdef16 ) {
	 	this.vdef16 = newVdef16;
	} 	  
	/**
	 * 属性vdef2的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return UFDouble
	 */
	public UFDouble getVdef2 () {
		return vdef2;
	}   
	/**
	 * 属性vdef2的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newVdef2 UFDouble
	 */
	public void setVdef2 (UFDouble newVdef2 ) {
	 	this.vdef2 = newVdef2;
	} 	  
	/**
	 * 属性dates的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return UFDate
	 */
	public UFDate getDates () {
		return dates;
	}   
	/**
	 * 属性dates的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newDates UFDate
	 */
	public void setDates (UFDate newDates ) {
	 	this.dates = newDates;
	} 	  
	/**
	 * 属性vdef5的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return UFDouble
	 */
	public UFDouble getVdef5 () {
		return vdef5;
	}   
	/**
	 * 属性vdef5的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newVdef5 UFDouble
	 */
	public void setVdef5 (UFDouble newVdef5 ) {
	 	this.vdef5 = newVdef5;
	} 	  
	/**
	 * 属性weight的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return UFDouble
	 */
	public UFDouble getWeight () {
		return weight;
	}   
	/**
	 * 属性weight的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newWeight UFDouble
	 */
	public void setWeight (UFDouble newWeight ) {
	 	this.weight = newWeight;
	} 	  
	/**
	 * 属性vdef19的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return String
	 */
	public String getVdef19 () {
		return vdef19;
	}   
	/**
	 * 属性vdef19的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newVdef19 String
	 */
	public void setVdef19 (String newVdef19 ) {
	 	this.vdef19 = newVdef19;
	} 	  
	/**
	 * 属性pk_invbasdoc的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return String
	 */
	public String getPk_invbasdoc () {
		return pk_invbasdoc;
	}   
	/**
	 * 属性pk_invbasdoc的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newPk_invbasdoc String
	 */
	public void setPk_invbasdoc (String newPk_invbasdoc ) {
	 	this.pk_invbasdoc = newPk_invbasdoc;
	} 	  
	/**
	 * 属性memo的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return String
	 */
	public String getMemo () {
		return memo;
	}   
	/**
	 * 属性memo的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newMemo String
	 */
	public void setMemo (String newMemo ) {
	 	this.memo = newMemo;
	} 	  
	/**
	 * 属性vdef18的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return String
	 */
	public String getVdef18 () {
		return vdef18;
	}   
	/**
	 * 属性vdef18的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newVdef18 String
	 */
	public void setVdef18 (String newVdef18 ) {
	 	this.vdef18 = newVdef18;
	} 	  
	/**
	 * 属性vdef4的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return UFDouble
	 */
	public UFDouble getVdef4 () {
		return vdef4;
	}   
	/**
	 * 属性vdef4的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newVdef4 UFDouble
	 */
	public void setVdef4 (UFDouble newVdef4 ) {
	 	this.vdef4 = newVdef4;
	} 	  
	/**
	 * 属性balancetype的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return String
	 */
	public String getBalancetype () {
		return balancetype;
	}   
	/**
	 * 属性balancetype的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newBalancetype String
	 */
	public void setBalancetype (String newBalancetype ) {
	 	this.balancetype = newBalancetype;
	} 	  

	
	
	public String getTimes() {
		return times;
	}
	public void setTimes(String times) {
		this.times = times;
	}
	/**
	 * 属性vdef17的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return String
	 */
	public String getVdef17 () {
		return vdef17;
	}   
	/**
	 * 属性vdef17的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newVdef17 String
	 */
	public void setVdef17 (String newVdef17 ) {
	 	this.vdef17 = newVdef17;
	} 	  
	/**
	 * 属性vdef1的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return UFDouble
	 */
	public UFDouble getVdef1 () {
		return vdef1;
	}   
	/**
	 * 属性vdef1的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newVdef1 UFDouble
	 */
	public void setVdef1 (UFDouble newVdef1 ) {
	 	this.vdef1 = newVdef1;
	} 	  
	/**
	 * 属性vdef20的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return String
	 */
	public String getVdef20 () {
		return vdef20;
	}   
	/**
	 * 属性vdef20的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newVdef20 String
	 */
	public void setVdef20 (String newVdef20 ) {
	 	this.vdef20 = newVdef20;
	} 	  
	/**
	 * 属性vdef8的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return UFDouble
	 */
	public UFDouble getVdef8 () {
		return vdef8;
	}   
	/**
	 * 属性vdef8的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newVdef8 UFDouble
	 */
	public void setVdef8 (UFDouble newVdef8 ) {
	 	this.vdef8 = newVdef8;
	} 	  
	/**
	 * 属性vfree4的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return String
	 */
	public String getVfree4 () {
		return vfree4;
	}   
	/**
	 * 属性vfree4的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newVfree4 String
	 */
	public void setVfree4 (String newVfree4 ) {
	 	this.vfree4 = newVfree4;
	} 	  
	/**
	 * 属性sellinfocode的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return String
	 */
	public String getSellinfocode () {
		return sellinfocode;
	}   
	/**
	 * 属性sellinfocode的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newSellinfocode String
	 */
	public void setSellinfocode (String newSellinfocode ) {
	 	this.sellinfocode = newSellinfocode;
	} 	  
	/**
	 * 属性balanceprice的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return UFDouble
	 */
	public UFDouble getBalanceprice () {
		return balanceprice;
	}   
	/**
	 * 属性balanceprice的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newBalanceprice UFDouble
	 */
	public void setBalanceprice (UFDouble newBalanceprice ) {
	 	this.balanceprice = newBalanceprice;
	} 	  
	/**
	 * 属性vfree5的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return String
	 */
	public String getVfree5 () {
		return vfree5;
	}   
	/**
	 * 属性vfree5的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newVfree5 String
	 */
	public void setVfree5 (String newVfree5 ) {
	 	this.vfree5 = newVfree5;
	} 	  
	/**
	 * 属性gatheringcode的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return String
	 */
	public String getGatheringcode () {
		return gatheringcode;
	}   
	/**
	 * 属性gatheringcode的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newGatheringcode String
	 */
	public void setGatheringcode (String newGatheringcode ) {
	 	this.gatheringcode = newGatheringcode;
	} 	  
	/**
	 * 属性vdef13的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return String
	 */
	public String getVdef13 () {
		return vdef13;
	}   
	/**
	 * 属性vdef13的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newVdef13 String
	 */
	public void setVdef13 (String newVdef13 ) {
	 	this.vdef13 = newVdef13;
	} 	  
	/**
	 * 属性vfree3的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return String
	 */
	public String getVfree3 () {
		return vfree3;
	}   
	/**
	 * 属性vfree3的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newVfree3 String
	 */
	public void setVfree3 (String newVfree3 ) {
	 	this.vfree3 = newVfree3;
	} 	  
	/**
	 * 属性pk_salescontract的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return String
	 */
	public String getPk_salescontract () {
		return pk_salescontract;
	}   
	/**
	 * 属性pk_salescontract的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newPk_salescontract String
	 */
	public void setPk_salescontract (String newPk_salescontract ) {
	 	this.pk_salescontract = newPk_salescontract;
	} 	  
	/**
	 * 属性poundcode的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return String
	 */
	public String getPoundcode () {
		return poundcode;
	}   
	/**
	 * 属性poundcode的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newPoundcode String
	 */
	public void setPoundcode (String newPoundcode ) {
	 	this.poundcode = newPoundcode;
	} 	  
	/**
	 * 属性vfree1的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return String
	 */
	public String getVfree1 () {
		return vfree1;
	}   
	/**
	 * 属性vfree1的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newVfree1 String
	 */
	public void setVfree1 (String newVfree1 ) {
	 	this.vfree1 = newVfree1;
	} 	  
	/**
	 * 属性vdef11的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return String
	 */
	public String getVdef11 () {
		return vdef11;
	}   
	/**
	 * 属性vdef11的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newVdef11 String
	 */
	public void setVdef11 (String newVdef11 ) {
	 	this.vdef11 = newVdef11;
	} 	  
  
	/**
	 * 属性num的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return UFDouble
	 */
	public UFDouble getNum () {
		return num;
	}   
	/**
	 * 属性num的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newNum UFDouble
	 */
	public void setNum (UFDouble newNum ) {
	 	this.num = newNum;
	} 	  
	/**
	 * 属性balancemny的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return UFDouble
	 */
	public UFDouble getBalancemny () {
		return balancemny;
	}   
	/**
	 * 属性balancemny的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newBalancemny UFDouble
	 */
	public void setBalancemny (UFDouble newBalancemny ) {
	 	this.balancemny = newBalancemny;
	} 	  
	/**
	 * 属性vdef12的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return String
	 */
	public String getVdef12 () {
		return vdef12;
	}   
	/**
	 * 属性vdef12的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newVdef12 String
	 */
	public void setVdef12 (String newVdef12 ) {
	 	this.vdef12 = newVdef12;
	} 	  
	/**
	 * 属性vdef3的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return UFDouble
	 */
	public UFDouble getVdef3 () {
		return vdef3;
	}   
	/**
	 * 属性vdef3的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newVdef3 UFDouble
	 */
	public void setVdef3 (UFDouble newVdef3 ) {
	 	this.vdef3 = newVdef3;
	} 	  
	/**
	 * 属性vdef6的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return UFDouble
	 */
	public UFDouble getVdef6 () {
		return vdef6;
	}   
	/**
	 * 属性vdef6的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newVdef6 UFDouble
	 */
	public void setVdef6 (UFDouble newVdef6 ) {
	 	this.vdef6 = newVdef6;
	} 	  
	/**
	 * 属性dr的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return UFDouble
	 */
	public Integer getDr () {
		return dr;
	}   
	/**
	 * 属性dr的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newDr UFDouble
	 */
	public void setDr ( Integer newDr ) {
	 	this.dr = newDr;
	} 	  
	/**
	 * 属性pk_invmandoc的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return String
	 */
	public String getPk_invmandoc () {
		return pk_invmandoc;
	}   
	/**
	 * 属性pk_invmandoc的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newPk_invmandoc String
	 */
	public void setPk_invmandoc (String newPk_invmandoc ) {
	 	this.pk_invmandoc = newPk_invmandoc;
	} 	  
	/**
	 * 属性pk_balancelist_b的Getter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @return String
	 */
	public String getPk_balancelist_b () {
		return pk_balancelist_b;
	}   
	/**
	 * 属性pk_balancelist_b的Setter方法.
	 * 创建日期:2011-06-23 15:23:29
	 * @param newPk_balancelist_b String
	 */
	public void setPk_balancelist_b (String newPk_balancelist_b ) {
	 	this.pk_balancelist_b = newPk_balancelist_b;
	} 	  
 
	/**
	  * <p>取得父VO主键字段.
	  * <p>
	  * 创建日期:2011-06-23 15:23:29
	  * @return java.lang.String
	  */
	public java.lang.String getParentPKFieldName() {
		return "pk_sellpaymentjs";
	}   
    
	/**
	  * <p>取得表主键.
	  * <p>
	  * 创建日期:2011-06-23 15:23:29
	  * @return java.lang.String
	  */
	public java.lang.String getPKFieldName() {
	  return "pk_balancelist_b";
	}
    
	/**
	 * <p>返回表名称.
	 * <p>
	 * 创建日期:2011-06-23 15:23:29
	 * @return java.lang.String
	 */
	public java.lang.String getTableName() {
		return "gt_balancelist_b";
	}    
    
    /**
	  * 按照默认方式创建构造子.
	  *
	  * 创建日期:2011-06-23 15:23:29
	  */
     public BalanceListBVO() {
		super();	
	}    
     
} 
