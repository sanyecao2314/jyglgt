...
//import nc.vo.so.sale.SaleexecuteVO;
...
public abstract class GeneralBillClientUI extends ToftPanel implements
  ...
  //add by zyb 销售出库修改的VO
  private GeneralBillVO m_editVOBill = null;
...
//行计数器
  ...
  public void afterEdit(nc.ui.pub.bill.BillEditEvent e) {
    ...
    if (sItemKey.equals("vbillcode")) {
      ...
      //add by zyb 编辑销售订单号
//    }else if(sItemKey.equals("vfree7")){
//    	getEditCtrl().afterVfree7Edit(e);
    }//end by zyb 
    /**
     * add by guoyt 
     * add date 2012-04-11 
     ...
     * 产成品入库单控制，其他不控制
     */
    else if(sItemKey.equals("vuserdef19") 
    		|| sItemKey.equals("vfree0")
    		|| sItemKey.equals("vfree1") 
    		|| sItemKey.equals("vfree2")
    		|| sItemKey.equals("vfree3")){
    	if(sItemKey.equals("vfree0"))
    		afterFreeItemEdit(e);
    	if(sItemKey.equals("vuserdef19"))
    		getEditCtrl().afterVuserDefEdit(e);
    	if(sItemKey.equals("vfree1") 
        		|| sItemKey.equals("vfree2")
        		|| sItemKey.equals("vfree3")){
    		getEditCtrl().afterElseDefaultEdit(e);
    	}
    	if(getBillType().equals("46")){
	    	 Object obj1 = getBillCardPanel().getBodyValueAt(row, "vfree1");
	    	 Object obj2 = getBillCardPanel().getBodyValueAt(row, "vfree2");
	    	 Object obj3 = getBillCardPanel().getBodyValueAt(row, "vfree3");
	    	 Object obj4 = getBillCardPanel().getBodyValueAt(row, "vuserdef19");
	    	 if(!Toolkits.isEmpty(obj1) && !Toolkits.isEmpty(obj2) && !Toolkits.isEmpty(obj3) && !Toolkits.isEmpty(obj4)){
	    		 UFDouble hsl = (new UFDouble(obj1.toString()))
	    		 				.multiply(new UFDouble(obj2.toString()))
	    		 				.multiply(new UFDouble(obj3.toString()))
	    		 				.multiply(new UFDouble(obj4.toString()))
	    		 				.div(1000000000);
	    		 getBillCardPanel().setBodyValueAt(new UFDouble(hsl.doubleValue(),3), row, "hsl");
...
//      2008.01.23 cy 雨润要求表头单据日期修改后，表体业务日期与表头单据日期一致 begin
  ...
  * @date 2011-8-27 上午08:19:14 
  * @return GeneralBillItemVO[]    返回类型 
  * @throws
   */
//  public GeneralBillItemVO[] getUpFreeItem() throws BusinessException{
//	  GeneralBillItemVO item = null;
//	  GeneralBillItemVO[] itemvos = null;	  
//	  ArrayList list = null;
//	  
//	  String cdilivertypeid = getBillCardPanel().getHeadItem("cdilivertypeid").getValueObject()
//	  ==null?"":getBillCardPanel().getHeadItem("cdilivertypeid").getValueObject().toString();
//	  
//	  StringBuffer sbf = new StringBuffer("");
//	  sbf.append(" select b.vfree1, b.vfree2, b.vfree3,b.vdef12, b.vfree4,b.vfree5,b.vdef12,b.pk_invmandoc,b.fnum,b.pk_deliveryplan_b ") 
//	  .append("   from gt_pcbill a, gt_pcbill_b b ") 
//	  .append("  where a.pk_pcbill = b.pk_pcbill ") 
//	  .append("    and nvl(a.dr, 0) = 0 ") 
//	  .append("    and nvl(b.dr, 0) = 0 and a.pk_pcbill='"+cdilivertypeid+"'"); 
//	  
//	  list = Proxy.getIGTItf().queryArrayBySql(sbf.toString());
//	  
//	  
//	  if(list.size()>0){
//		  itemvos= new GeneralBillItemVO[list.size()];
//	  for (int i = 0; i < list.size(); i++) {
//		  item =new GeneralBillItemVO();
...
//		  Object vfree5 = getBillCardPanel().getBodyValueAt(i, "vfree5");
//		  Object cqualitylevelid = getBillCardPanel().getBodyValueAt(i, "cqualitylevelid");
//		  Object nshouldoutassistnum = getBillCardPanel().getBodyValueAt(i, "nshouldoutassistnum");
//		  Object vuserdef20 = getBillCardPanel().getBodyValueAt(i, "vuserdef20");
//		  
//		  if(!Toolkits.isEmpty(map.get("vfree1"))){
//			  item.setVfree1(map.get("vfree1").toString());
//		  }
//		  if(!Toolkits.isEmpty(map.get("vfree2"))){
//			  item.setVfree2(map.get("vfree2").toString());
//		  }		  
//		  if(!Toolkits.isEmpty(map.get("vfree3"))){
//			  item.setVfree3(map.get("vfree3").toString());
//		  }
//		  if(!Toolkits.isEmpty(map.get("vfree4"))){
//			  item.setVfree4(map.get("vfree4").toString());
//		  }
//		  if(!Toolkits.isEmpty(map.get("vfree5"))){
//			  item.setVfree5(map.get("vfree5").toString());
//		  }
//		  if(!Toolkits.isEmpty(map.get("vdef12"))){
//			  item.setVuserdef8(map.get("vdef12").toString());
//		  }
//		  if(!Toolkits.isEmpty(map.get("pk_invmandoc"))){
//			  item.setCinvmanid(map.get("pk_invmandoc").toString());
//		  }
//		  if(!Toolkits.isEmpty(map.get("fnum"))){
//			  item.setNshouldoutassistnum(Toolkits.getUFDouble(map.get("fnum")));
//		  }
//		  if(!Toolkits.isEmpty(map.get("pk_deliveryplan_b"))){
//			  item.setVuserdef10(map.get("pk_deliveryplan_b").toString());
//		  }
//		  
//		  itemvos[i]=item;
//	}}
...
//    二次开发扩展
    	...
    	String vuserdef19 = "";
    	String pcpk = "";
     	if(getBillType().equals("4C")){
     		pcpk = getBillCardPanel().getHeadItem("cdilivertypeid").getValueObject()
  	         ==null?"":getBillCardPanel().getHeadItem("cdilivertypeid").getValueObject().toString();
           if(bo.getName().equals("修改")){
  	  	pk_pcbill = Toolkits.getString(getBillCardPanel().getHeadItem("cdilivertypeid").getValueObject());
  		  }else if(bo.getName().equals("复制")){
    	  	if(!vuserdef19.equals("")||vuserdef19.length()>0){
    	  	    showErrorMessage("该销售出库单已经过磅单，不能 复制!");
    	  	  	return;
    	  	  		}
    	  		  }//end by zyb 2011-09-04过磅的出库单不能删除，修改，复制
  		  
  	}
     	//自动化系统生成的数据不可以修改  add by shipeng at 2014-10-30
     	if(getBillType().equals("45")){
     		if(bo.getName().equals("修改")){
     			String vuserdef20 = Toolkits.getString(getBillCardPanel().getHeadItem("vuserdef20").getValueObject());
     			if(!vuserdef20.equals("")){
	    	  	    showErrorMessage("自动化系统生成的数据不可以修改!");
	    	  	  	return;
    	  	  	}
     		}  
  	    }
  ...
  public void onAdd(boolean bUpataBotton, GeneralBillVO voBill) {
    ...
    try {
      ...
      if (voBill == null) {
    	...
    	// add by wangxhi 河北新金 防止新增时得到光标后自动带入前张单据的值 2011-10-24 begin
//        GeneralBillUICtl.clearHeadTailValueForUserdefRef(this);
		// add by wangxhi 河北新金 防止新增时得到光标后自动带入前张单据的值 2011-10-24 end
      ...
      //add by zyb 销售出库新增，设置派车单可编辑,避免修改设置不可编辑后，新增依然为不可编辑
      if(getBillType().equals("4C")){
    		getBillCardPanel().getHeadItem("cdilivertypeid").setEdit(true);
      }//end by zyb
...
//          .setEnabled(true);
  ...
  protected void setBodyInvValue(int row, InvVO voInv) {
    ...
    try {
      ...
      //zyb
//      if(getBillType().equals("4C")){
//      HashMap<String, Object> pchm = getHsPrice(voInv.getCinventoryid());
      
//      getBillCardPanel().setBodyValueAt(Toolkits.getUFDouble(pchm.get("hsprice")), row, "nprice");
//      getBillCardPanel().setBodyValueAt(Toolkits.getString(pchm.get("pk_deliveryplan_b")), row, "vuserdef20");}
    ...
    * @功能: 得到含税单价
    * @作者 zyb  
    * @创建时间 2011-7-26 下午03:50:15
    * @throws
     */
//	public HashMap<String, Object> getHsPrice(String pk_invmandoc){
//		Object pk_pcbill = getBillCardPanel().getHeadItem("cdilivertypeid").getValueObject();
//		UFDouble hsprice = new UFDouble(0);
//		if(pk_pcbill!=null){
//		StringBuffer sbf = new StringBuffer("");
//		sbf.append("  select gt_pcbill_b.hsprice,gt_pcbill_b.pk_deliveryplan_b from gt_pcbill inner join gt_pcbill_b on gt_pcbill_b.pk_pcbill=gt_pcbill.pk_pcbill ") 
//		.append("  where gt_pcbill.pk_pcbill='"+pk_pcbill.toString()+"' and gt_pcbill_b.pk_invmandoc='"+pk_invmandoc+"' and nvl(gt_pcbill.dr,0)=0 and nvl(gt_pcbill_b.dr,0)=0"); 
//		ArrayList obj=null;
//		try {
//			obj = Proxy.getIGTItf().queryArrayBySql(sbf.toString());
//		} catch (BusinessException e) {
//			// TODO Auto-generated catch block
//			e.printStackTrace();
//		}
//		if(obj.size()>0&&obj!=null){
//			HashMap<String, Object> hm = (HashMap<String, Object>)obj.get(0);
//			return hm;
//		}
//		}
//		return null;
...
//  二次开发扩展
  ...
  public boolean beforeEdit(nc.ui.pub.bill.BillItemEvent e) {
    if(!getPluginProxy().beforeEdit(e))
      return false;
    //参照刷新 add by shipeng at 2011-08-02
  ...
  public boolean beforeEdit(nc.ui.pub.bill.BillEditEvent e) {
    ...
    if(!getPluginProxy().beforeEdit(e))
      bret = false;
    //add by zyb编辑表体销售订单号前的过滤
    if(getBillType().equals("4C")){
	    if(e.getKey().equals("vfree7")&&e.getPos()==1){
	    	beforeEditVfree7(e);
	    }
    }//end by zyb
  ...
  * 功能 编辑表体销售订单号前的过滤
  * 作者 朱永波 
  * 时间 2012-2-3 上午11:01:25 
  * 返回 void    返回类型
   */
  private void beforeEditVfree7(BillEditEvent e){
	   String ccustomerid = Toolkits.getString(getBillCardPanel().getHeadItem("ccustomerid").getValueObject());
	   UIRefPane pane = (UIRefPane) getBillCardPanel().getBillModel().getItemByKey("vfree7").getComponent();
	   if(pane!=null){
		   pane.getRef().getRefModel().addWherePart(" and so_sale.ccustomerid='"+ccustomerid+"'", true);
	   }   
  }
...
/*  protected void resetButtons() {
   ...
   * 
  * @Title: beforeSave 
  * @Description:销售出库检查自由项和等级是否和派车单一致,控制派车单和销售发货件数相等,edit by zyb 写在脚本里面前台废弃
  * @author zyb zhyb966@gmail.com 
  * @date 2011-8-27 上午08:28:01 
  * @return void    返回类型 
  * @throws
   */
  public GeneralBillVO beforesave(GeneralBillVO voBill) throws BusinessException{
      //add by zyb 回写派车单
      if(getBillType().equals("4C")){
//    	GeneralBillItemVO[] upitemvo = getUpFreeItem();
    	  GeneralBillHeaderVO hvo = getM_voBill().getHeaderVO();
    	GeneralBillItemVO[] itemvo = getM_voBill().getItemVOs();
    	StringBuffer sbf = new StringBuffer("以下质量等级");
    	for (int i = 0; i < itemvo.length; i++) {
    		String cdilivertypeid = hvo.getCdilivertypeid();
    		if(Toolkits.isEmpty(cdilivertypeid))
    			continue;
    		
    		String csourcebillbid = itemvo[i].getAttributeValue("csourcebillbid")==null?"":itemvo[i].getAttributeValue("csourcebillbid").toString();
    		if(Toolkits.isEmpty(csourcebillbid)){
    			continue;
    		} 		
    		//add by zyb 2012-03-22将当前输入的数量和辅数量设置给货位
    		UFDouble noutnum = itemvo[i].getNoutnum();
    		UFDouble noutassistnum = itemvo[i].getNoutassistnum();
    		LocatorVO[] lvo = itemvo[i].getLocator();
    		if(lvo!=null&&lvo.length>0){
	    		lvo[0].setNoutspaceassistnum(noutassistnum);
	    		lvo[0].setNoutspacenum(noutnum);
	    		itemvo[i].setLocator(lvo);
    		}
    		// end by zyb 将出口数量赋给货位 
    		int row = i+1;
    		
    		Object vbatchno = Toolkits.getString(itemvo[i].getVbatchcode());
    		if(Toolkits.isEmpty(vbatchno)){
    			throw new BusinessException("请先输入批次号!");
    		}
//    		String pk_invbasdoc = itemvo[i].getCinvbasid()==null?"":itemvo[i].getCinvbasid().toString(); 
//			StringBuffer stbf = new StringBuffer("");
//			stbf.append("  select qc_checkstate_b.ccheckstatename from scm_batchcode,qc_checkstate_b  ") 
//			.append("  where scm_batchcode.cqualitylevelid=qc_checkstate_b.ccheckstate_bid ") 
//			.append("   and scm_batchcode.vbatchcode='"+vbatchno.toString()+"' and nvl(scm_batchcode.dr,0)=0 and scm_batchcode.pk_invbasdoc='"+pk_invbasdoc+"' "); 
//
//			String cqualityid = Proxy.getIGTItf().queryStrBySql(stbf.toString());
			
//			String csourcebillbid = itemvo[i].getAttributeValue("csourcebillbid")==null?"":itemvo[i].getAttributeValue("csourcebillbid").toString();
//			SaleexecuteVO dbvo = (SaleexecuteVO) new HYPubBO().queryByPrimaryKey(SaleexecuteVO.class, csourcebillbid);
//			String vdef18 = dbvo.getVdef18()==null?"":dbvo.getVdef18().toString();//质量等级
//			if(!Toolkits.isEmpty(cqualityid)){
//			if(!vdef18.equals(cqualityid)){
//				sbf.append("第"+row+"行,");
//			}
//			}
		}
    	sbf.append("与订单质量等级不符，请检查!");
    	//如果质量等级与订单不符合，询问是否继续
    	if(sbf.toString().indexOf("第")!=-1){
    		int flag = showYesNoCancelMessage(""+sbf.toString()+"");
    		if(flag!=UIDialog.ID_YES){
    			throw new BusinessException(sbf.toString());
    		}
    	}
    	GeneralBillVO gbvo = new GeneralBillVO();
    	gbvo.setParentVO(hvo);
    	gbvo.setChildrenVO(itemvo);
    	return gbvo;
        }
  ...
  * @功能: 回写派车单
  * @作者 zyb
  * @创建时间 2011-7-21 下午03:03:42
  * @throws
   */
//  public void backPc(String pk_pcbill,String flag){
//	  
////--------------------------2012-8-28 --lzj
//	String pcsql = "select pk_address from gt_pcbill where nvl(dr,0)=0 and pk_pcbill='" + pk_pcbill + "'";
//	String pcbn= null;
//	try {
//		pcbn = Proxy.getIGTItf().queryStrBySql(pcsql);
//	} catch (BusinessException e) {
//		// TODO Auto-generated catch block
//		e.printStackTrace();
//	}
//	if(pcbn==null||pcbn.equals("控制计量")){
//		
//		String sql = "update gt_pcbill set flag_issue='"+flag+"' where nvl(dr,0)=0 and pk_pcbill='"+pk_pcbill+"';";
//		  try {
//			((IGTITF)Proxy.getIGTItf()).updateBYsql(sql);
//		} catch (BusinessException e) {
//			e.printStackTrace();
//		}
//
...
//  }
  ...
  * @功能: 修改派车单号后的回写派车单
  * @作者 朱永波
  * @日期 2011-10-9 下午02:55:44 
  * @返回类型 void
  * @throws
   */
  public void backPcWhenEdit(){
	  String pk_pcbillnow = Toolkits.getString(getBillCardPanel().getHeadItem("cdilivertypeid").getValueObject());
	  if(!pk_pcbillnow.equals(pk_pcbill)&&BillMode.Update == getM_iMode()){
//		  backPc(pk_pcbillnow, "Y");
		  //修改的派车单号不为空
		  if(!pk_pcbill.equals("")){
//		  backPc(pk_pcbill, "N");
		  }
		  pk_pcbill= "";
	  }else if(BillMode.New == getM_iMode()){
//		  backPc(pk_pcbillnow, "Y");
	  }
  }
...
//        voBill.setHeaderValue("time", m_dTime);
      ...
      if (BillMode.Update == getM_iMode()) {
        ...
        //回写派车单 add by zyb 避免出现已经回写但是没有保存
...
//        getButtonManager().getButton(ICButtonConst.BTN_BILL_COPY)
//            .setEnabled(true);
        //add by zyb 20120301 销售出库复制，可能导致回写出错所以禁用
        getButtonManager().getButton(ICButtonConst.BTN_BILL_COPY)
        .setEnabled(false);//end by zyb
        getButtonManager().getButton(ICButtonConst.BTN_BROWSE_LOCATE)
            .setEnabled(true);
...
/*      getButtonManager()
    ...
    if (BillMode.Card == getM_iCurPanel()) {
      ...
      getButtonManager().getButton(ICButtonConst.BTN_ASSIST_FUNC_ONHAND)
          .setEnabled(false);
      //add by zyb 2011-10-29 列表情况，编辑和删除不可用
      if(getBillType().equals("4C")){
    	  getButtonManager().getButton(ScmButtonConst.BTN_BILL_EDIT).setEnabled(false);
    	  getButtonManager().getButton(ScmButtonConst.BTN_BILL_DELETE).setEnabled(false);
      }//end by zyb 
    }
	   ...
	   // addied by liuzy 2010-12-9 上午11:27:03 如果是换算率记结存，需要把换算率带入单据
	    ...
	    if (voInv.getIsFreeItemMgt() != null
	      ...
	      if (freevo != null && freevo.getVfree0() != null) {
	        ...
	        //add 换算率 by zyb 2012-01-06 
	        //因为所有与库存有关系的都用到换算率，又不想勾选换算率结存，统一用批次带出的换算率
	        UFDouble fnum = Toolkits.getUFDouble(voLot.getNinassistnum());//辅数量
	        UFDouble num = voLot.getNinnum();
	        UFDouble hsl = new UFDouble(0);
	        if(fnum.doubleValue()>0){
	        	hsl = new UFDouble(num.div(fnum).doubleValue(),3);
		      	  Object srcItem = getBillCardPanel().getBodyItem("hsl");
		            Object oldHsl = getBillCardPanel().getBodyValueAt(irow, "hsl");
		            BillEditEvent e = new BillEditEvent(srcItem,hsl,oldHsl,"hsl",irow,BillItem.BODY);
		            getBillCardPanel().setBodyValueAt(hsl, irow, "hsl");
	        ...
	        if(getBillType().equals("4C")){
	            Object cdilivertypeid =getBillCardPanel().getHeadItem("cdilivertypeid").getValueObject();//派车单号
	        if(fnum.doubleValue()!=0){
	            num = hsl.multiply(fnum);         
	            getBillCardPanel().setBodyValueAt(fnum, irow, "noutassistnum");
	              getBillCardPanel().setBodyValueAt(num, irow, "noutnum");
	              getM_voBill().setItemValue(irow, "nshouldoutnum", fnum);
	              getM_voBill().setItemValue(irow, "noutnum", num);  
	            getBillCardPanel().getBillModel().setValueAt(num, irow, "nshouldoutnum");
	            getM_voBill().setItemValue(irow, "nshouldoutnum", num);
	            getBillCardPanel().getBillModel().setValueAt(fnum, irow, "nshouldoutassistnum");
	            getM_voBill().setItemValue(irow, "nshouldoutassistnum", fnum);
	            //2011-12-30zyb增加货位信息
	            Object vfree10 = freevo.getVfree10();
	            if(!Toolkits.isEmpty(vfree10)){
	              Object cwarehouseid=getBillCardPanel().getHeadItem("cwarehouseid").getValueObject();
//	          	  HashMap<String, String> map = getCspaceid(vfree10.toString(),cwarehouseid);
//	          	  if(map!=null){
//	      	    	  String cspaceid = map.get("pk_cargdoc");
//	      	    	  String csname = map.get("csname");
//	      	    	  getEditCtrl().setRowSpaceData(irow, cspaceid, csname);
//	      	    	  getBillCardPanel().getBillModel().execLoadFormula();
//	          	  }
	          	  
	            }//选择批次号根据层号带出货位2011-12-30 结束
	            getBillCardPanel().setBodyValueAt(ClientEnvironment.getInstance().getDate(), irow, "dbizdate");
	            getM_voBill().setItemValue(irow, "dbizdate", ClientEnvironment.getInstance().getDate());
	        }//end  2012-01-06 
	        }
	      }
	      getM_voBill().setItemFreeVO(irow, freevo);
	...
	* 功能 根据层号获得货位
	* 作者 朱永波 
	* 时间 2011-12-30 下午02:07:31 
	* 返回 String    返回类型
	 */
//	public HashMap<String, String> getCspaceid(String vfree10,Object cwarehouseid){
//		HashMap<String, String> hm = new HashMap<String, String>();
//		StringBuffer sbf = new StringBuffer("");
//		sbf.append("   select distinct c.csname, c.pk_cargdoc ") 
//		.append("     from ic_onhandnum a, ic_onhandnum_b b, bd_cargdoc c ") 
//		.append("    where a.pk_onhandnum = b.pk_onhandnum ") 
//		.append("      and b.cspaceid = c.pk_cargdoc ") 
//		.append("      and nvl(a.dr, 0) = 0 ") 
//		.append("      and nvl(b.dr, 0) = 0 ") 
//		.append("      and nvl(c.dr, 0) = 0 and a.vfree10='"+vfree10+"' and a.cwarehouseid='"+cwarehouseid+"' and b.nastnum>0"); 
//
//		try {
//			ArrayList list = Proxy.getIGTItf().queryArrayBySql(sbf.toString());
