...
//									.getBodyVOs(vos), getBillCardPanel(), getBillType());
					...
					//begin ncm  zhangweiaf 201311201043104574 2013-11-25_吉宇钢联_采购模块采购流程二开
					//如果当前单据是采购入库单，且单据数据不是空
					if("45".equals(getBillType()) && null != vos && 0 < vos.length){
						String[] rets = SourceRefDlg.getBillTypeAndBiz(bo);
						String srcBillType = null;
						if(rets!=null && rets.length>0){
							srcBillType = rets[0];
						}
						//如果来源单据是采购到货
						if("23".equals(srcBillType)){
							resetVOS((GeneralBillVO[])vos);
						}
					}
					//end ncm  zhangweiaf 201311201043104574 2013-11-25_吉宇钢联_采购模块采购流程二开
	 ...
	 * 重新设置采购入库单的部分字段值
	 */
	private void resetVOS(GeneralBillVO[]vos) throws Exception{

		if(null == vos || 0 == vos.length){
			return;
		}
		for (int i = 0; i < vos.length; i++) {
			GeneralBillHeaderVO headerVO = vos[i].getHeaderVO();
			if(null == headerVO){
				continue;
			}
			//货款结算依据
			String hkjsyj = (String)headerVO.getAttributeValue("vuserdef14");
			headerVO.setAttributeValue("vuserdef10", null);
			headerVO.setAttributeValue("vuserdef16", null);
			if(null == hkjsyj || "".equals(hkjsyj.trim())){
				continue;
			}
			GeneralBillItemVO[] items = vos[i].getItemVOs();
			if(null == items || 0 == items.length){
				continue;
			}
			for (int j = 0; j < items.length; j++) {
				UFDouble zero = new UFDouble(0);
				//货款结算数量，如果是发货方，取发货数量，如果是收货方，取实收数量
				if("发货方".equals(hkjsyj)){
					items[j].setAttributeValue("vuserdef26", items[j].getAttributeValue("vuserdef24"));
				}else{
					items[j].setAttributeValue("vuserdef26",items[j].getAttributeValue("ninnum"));
				}
				//货款结算数量
				UFDouble hkjssl = zero;
				if(isStringCast(items[j].getAttributeValue("vuserdef26"))){
					hkjssl = new UFDouble(items[j].getAttributeValue("vuserdef26").toString());
				}
				//结算单价
				UFDouble jsdj = zero;
				if(isStringCast(items[j].getAttributeValue("vuserdef27"))){
					jsdj = new UFDouble(items[j].getAttributeValue("vuserdef27").toString());
				}
				/*//将采购订单的含税单价保存到采购入库的结算单价
				items[j].setAttributeValue("vuserdef27",jsdj);*/
				//无税单价
				UFDouble wsdj = zero;
				if(isStringCast(items[j].getAttributeValue("nprice"))){//nprice保存的是采购订单的无税单价
					wsdj = new UFDouble(items[j].getAttributeValue("nprice").toString());
				}
				//金额
				items[j].setAttributeValue("vuserdef28",hkjssl.multiply(jsdj));
				//实收数量
				UFDouble sssl = zero;
				if(isStringCast(items[j].getAttributeValue("ninnum"))){
					sssl = new UFDouble(items[j].getAttributeValue("ninnum").toString());
				}
				if(sssl.compareTo(zero) == 0){
					throw new Exception("第" + String.valueOf(j + 1) + "行，实收数量是0，不能算出入库单价，请检查到货计量的净重");
				}
				//金额
				UFDouble jine = zero;
				/*if(isStringCast(items[j].getAttributeValue("vuserdef28"))){
					jine = new UFDouble(items[j].getAttributeValue("vuserdef28").toString());
				}*/
				if(isStringCast(items[j].getAttributeValue("vuserdef28"))){
					jine = new UFDouble(items[j].getAttributeValue("vuserdef28").toString());
				}
				//入库单价 = 金额/实收数量
				items[j].setAttributeValue("vuserdef31",jine.div(sssl));
				UFDouble rkdj = zero;
				if(isStringCast(items[j].getAttributeValue("vuserdef31"))){
					rkdj = new UFDouble(items[j].getAttributeValue("vuserdef31").toString());
				}
				//入库单上原来的单价作为入库无税单价
				items[j].setAttributeValue("nprice",rkdj.multiply(wsdj).div(jsdj));
				UFDouble nprice = zero;
				if(isStringCast(items[j].getAttributeValue("nprice"))){
					nprice = new UFDouble(items[j].getAttributeValue("nprice").toString());
				}
				//入库单上原来的金额字段 = 无税金额 = 实收数量 * 入库无税单价
				items[j].setAttributeValue("nmny",sssl.multiply(nprice));
				//无税金额
				items[j].setAttributeValue("vuserdef29",sssl.multiply(nprice));
				/*//无税金额
				UFDouble wsjine = zero;
				if(isStringCast(items[j].getAttributeValue("nmny"))){
					wsjine = new UFDouble(items[j].getAttributeValue("nmny").toString());
				}*/
				//运费结算数量
				items[j].setAttributeValue("vuserdef30",zero);
				//运费单价
				items[j].setAttributeValue("vuserdef32",zero);
				//运费无税单价
				items[j].setAttributeValue("vuserdef33",zero);
				//运费金额
				items[j].setAttributeValue("vuserdef34",zero);
				//运费无税金额
				items[j].setAttributeValue("vuserdef35",zero);
				//超路耗数量
				items[j].setAttributeValue("vuserdef36",zero);
				//超路耗单价
				items[j].setAttributeValue("vuserdef37",zero);
				//超路耗无税单价
				items[j].setAttributeValue("vuserdef38",zero);
				//超路耗金额
				items[j].setAttributeValue("vuserdef39",zero);
				//超路耗无税金额
				items[j].setAttributeValue("vuserdef40",zero);
				//运费扣款
				items[j].setAttributeValue("vuserdef41",zero);
				//入库日期，默认当前日期
				items[j].setAttributeValue("dbizdate",new UFDate(new Date()));
				//上游的到货单表体自定义项10,11,12,13,14,15,16有值，但是下游的采购入库单不需要这些值，且保存的时候报错
				items[j].setAttributeValue("vuserdef10",null);
				items[j].setAttributeValue("vuserdef11",null);
				items[j].setAttributeValue("vuserdef12",null);
				items[j].setAttributeValue("vuserdef13",null);
				items[j].setAttributeValue("vuserdef14",null);
				items[j].setAttributeValue("vuserdef15",null);
				items[j].setAttributeValue("vuserdef16",null);
			}
		}
	}
	
	private boolean isStringCast(Object obj){
		if(null == obj){
			return false;
		}
