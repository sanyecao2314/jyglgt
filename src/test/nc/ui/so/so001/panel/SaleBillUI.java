...
public abstract class SaleBillUI extends SaleBillListUI implements IFreshTsListener, ILinkAdd,
	...
	private boolean calculatePreceive(SaleOrderVO saleorder) {
		...
		if (bi == null) {
			SCMEnv.out("错误：预收款比例在模版中不存在!"); /*-=notranslate=-*/
			return true;
		}
		bi = getBillCardPanel().getHeadItem("npreceivemny");
		if (bi == null) {
			SCMEnv.out("错误：预收款金额在模版中不存在!"); /*-=notranslate=-*/
			return true;
		}
		...
		if (temp == null && npreceiverate != null && "比例".equals(SO_21)) { /*-=notranslate=-*/
	...
	public void handleException(Exception e, String msg) {
		MessageDialog.showWarningDlg(this, "Warning", msg + e.getMessage()); 
	...
	protected void initButtons() {
		...
		if (strState.equals("BOM")) {
			...
			if (boBusiType.getChildButtonGroup() != null
				...
				SCMEnv.out("没有初始化业务类型!"); /*-=notranslate=-*/
			}
	...
	private void initInvList() {
		...
		try {
			...
			SCMEnv.out("自由项设置失败!"); /*-=notranslate=-*/
		}
	}
	...
	public void loadCardData() {
		try {
			...
			SCMEnv.out("数据读取[共用时" + (System.currentTimeMillis() - s) + "]"); /*-=notranslate=-*/
			...
			// 加载是否打印合计
			copyIstotal();
			SCMEnv.out("数据设置[共用时" + (System.currentTimeMillis() - s) + "]"); /*-=notranslate=-*/
			long s1 = System.currentTimeMillis();

			getBillCardPanel().getBillModel().execLoadFormula();

			SCMEnv.out("执行公式[共用时" + (System.currentTimeMillis() - s1) + "]"); /*-=notranslate=-*/
			...
			SCMEnv.out("加载表头[共用时" + (System.currentTimeMillis() - s1) + "]"); /*-=notranslate=-*/
			...
			SCMEnv.out("加载表体[共用时" + (System.currentTimeMillis() - s1) + "]"); /*-=notranslate=-*/
	...
	* @功能:  判断是否被发货计算引用
	* @作者 zyb  
	* @创建时间 2011-7-27 下午03:22:53
	* @throws
	 */
	private boolean isBillDown(String pk){
		String sql = "select count(pk_saleorder_b)  from gt_deliveryplan_b where pk_saleorder ='"+pk+"' and nvl(dr,0)=0";
		Object count=null;
		boolean flag = false;
		try {
			count = Proxy.getItf().queryStrBySql(sql);
			int num  = Toolkits.getInteger(count); 
			if(num>0){
				flag =true;
			}
		} catch (BusinessException e) {
	...
	public void loadCardData(SaleOrderVO billvo) {
		...
		try {
			...
			execBodyLoadFormula("卡片"); /*-=notranslate=-*/
			//getBillCardPanel().getBillModel().execLoadFormula();
			...
			SCMEnv.out("加载表头数据bo[共用时" + (System.currentTimeMillis() - s) + "]"); /*-=notranslate=-*/
			s = System.currentTimeMillis();
...
//			}
			...
			SCMEnv.out("加载表体数据bo[共用时" + (System.currentTimeMillis() - s) + "]"); /*-=notranslate=-*/
...
/*			// 重新加载表头TS
	...
	protected void onAction(ButtonObject bo) {
		...
		if (bo.getTag().equals("SoBlankout")) {
			// 存在有效数据
			if ((strShowState.equals("列表") && getBillListPanel().getHeadTable() /*-=notranslate=-*/
					.getSelectedRowCount() >= 1)
					|| strShowState.equals("卡片")) { /*-=notranslate=-*/
				// 提示
		...
		try {
			...
			if (bo.getTag().equals("SoBlankout")) {
				...
				//计划未关闭或未删除，不准许关闭订单 add by shipeng\ship\sp\施鹏 at 2012-2-8
				if(!isPlanClosing()){
					MessageDialog.showErrorDlg(this, "订单关闭", "当前订单已有发货计划使用，关闭失败！");
					return;
				}//end by shipeng\ship\sp\施鹏  at 2012-2-8
				saleorder.setIAction(ISaleOrderAction.A_CLOSE);			
				for (int i = 0, loop = ordbvo.length; i < loop; i++) {
					// 设置atp更新动作，以便更新ATP
					ordbvo[i].setIAction(ISaleOrderAction.A_OUTEND);
				}
				//add by shipeng\ship\sp\施鹏 at 2011-9-22
				String vdef16  = getBillCardPanel().getHeadItem("vdef16")==null?"":getBillCardPanel().getHeadItem("vdef16").getValueObject().toString();
				String[] vbillcodes=null;
//				if(vdef16!=null && vdef16.equals("产成品销售")){
//					vbillcodes=pccode();
//					if(vbillcodes!=null){
//					   int result=MessageDialog.showYesNoCancelDlg(this, "产成品关闭", "以下已失效的派车单已过皮，是否强制关闭派车单，\n派车单号为："+Toolkits.combinArrayToString(vbillcodes));
//					   if(result != UIDialog.ID_YES){
//						   return;
//					   }
//					}
					saleorder.getHeadVO().setVdef8(getClientEnvironment().getUser().getPrimaryKey());//关闭人
					saleorder.getHeadVO().setVdef9(getClientEnvironment().getServerTime().toString());//关闭时间
//				}
				//end by shipeng\ship\sp\施鹏 at 2011-9-22
			...
			if (bo.getTag().equals("APPROVE")) {
				...
				nc.vo.scm.pub.SCMEnv.out("审核检测通过"); /*-=notranslate=-*/
			}
			if (bo.getTag().equals("SoUnApprove")) {
				onUnApproveCheck(saleorder);
			}

			nc.vo.scm.pub.SCMEnv.out("<====== 销售订单:[" + bo.getName() + "]动作 ## 调用脚本前准备时间共耗时[" /*-=notranslate=-*/
					+ (System.currentTimeMillis() - s_init) / 1000 + "."
					+ (System.currentTimeMillis() - s_init) % 1000 + "秒]==============>"); /*-=notranslate=-*/
				...
				while (bContinue) {
					...
					try {
						...
						if (obj!=null&& UFBoolean.valueOf(obj.toString()).booleanValue())
							MessageDialog.showHintDlg(this, NCLangRes.getInstance().getStrByID("40060302", "UPP40060301-100058"), 
									NCLangRes.getInstance().getStrByID("40060302", "UPP40060301-100059")
									+ saleorder.getHeadVO().getVreceiptcode()
									+ NCLangRes.getInstance().getStrByID("40060302", "UPP40060301-100060"));
			...
			nc.vo.scm.pub.SCMEnv.out("<====== 销售订单:[" + bo.getName() + "]动作 ## 调用脚本过程共耗时[" /*-=notranslate=-*/
					+ (System.currentTimeMillis() - s1) / 1000 + "."
					+ (System.currentTimeMillis() - s1) % 1000 + "秒]==============>"); /*-=notranslate=-*/
			...
			if (PfUtilClient.isSuccess()) {
				...
				nc.vo.scm.pub.SCMEnv.out("<====== 销售订单:[" + bo.getName() + "]动作 ## 调用脚本后刷新界面共耗时[" /*-=notranslate=-*/
						+ (System.currentTimeMillis() - s1) / 1000 + "."
						+ (System.currentTimeMillis() - s1) % 1000 + "秒]==============>"); /*-=notranslate=-*/
		...
		nc.vo.scm.pub.SCMEnv.out("<====== 销售订单:[" + bo.getName() + "]动作 ## 共耗时[" /*-=notranslate=-*/
				+ (System.currentTimeMillis() - s_init) / 1000 + "."
				+ (System.currentTimeMillis() - s_init) % 1000 + "秒]==============>"); /*-=notranslate=-*/
	...
	* 功能 订单是否做下线计量
	* 作者 朱永波 
	* 时间 2012-3-18 下午04:09:05 
	* 返回 boolean    返回类型
	 */
	public boolean isXx(String orderid) {
		boolean flag = false;
		String sql = "select count(*) from gt_trackhmeter where gt_trackhmeter.custorderno='"+orderid+"' and nvl(dr,0)=0";
		
		IUAPQueryBS qbs = (IUAPQueryBS) NCLocator.getInstance().lookup(IUAPQueryBS.class.getName());
		
		int count=0;;
		try {
			count = qbs.executeQuery(sql, new ColumnProcessor())==null?new Integer(0):new Integer(qbs.executeQuery(sql, new ColumnProcessor()).toString());
	...
	* 返回 boolean    返回类型
	 */
	public boolean isHaveSk(String orderid) {
		boolean flag = false;

		String sql = "select count(*) from gt_credit where csaleid='"+orderid+"' and nvl(dr,0)=0";
		...
		int countsk=0;
		try {
			countsk = qbs.executeQuery(sql, new ColumnProcessor())==null?new Integer(0):new Integer(qbs.executeQuery(sql, new ColumnProcessor()).toString());
		} catch (NumberFormatException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (BusinessException e) {
	...
	public int onBlankout(SaleOrderVO vo, String tag) throws Exception {
		...
		if (vo.getChildrenVO() != null) {
			for (int i = 0; i < vo.getChildrenVO().length; i++) {
				vo.getChildrenVO()[i].setStatus(VOStatus.DELETED);
			}
		}
	...
	private void onUnApproveCheck(SaleOrderVO saleorder) throws nc.vo.pub.ValidationException {
		if (saleorder.getHeadVO().getAttributeValue("bcooptopo") != null
				&& ((UFBoolean) saleorder.getHeadVO().getAttributeValue("bcooptopo"))
						.booleanValue()) {
			throw new ValidationException(NCLangRes.getInstance().getStrByID("40060302", "UPP40060301-100061"));
		}
		...
		for (int i = 0; i < oldbodyVOs.length; i++) {
			...
			if(getBillType().equals("30")){
				//add by zyb 下线信息和收款通知引用不能弃审
//				if(isXx(oldbodyVOs[i].getCsaleid())){
//					throw new ValidationException("该订单被下线信息录入引用!");
//				}
//				if(isHaveSk(oldbodyVOs[i].getCsaleid())){
//					throw new ValidationException("该订单被收款通知引用引用!");
//				}//end by zyb 20120318
//				if(isBillDown(oldbodyVOs[i].getCsaleid())){
//					throw new ValidationException("该订单已经被发货计划引用");
//				}
//				if(isUnPass()){
//					throw new ValidationException("该订单已经过磅!");
//				}
				if(isPc()){
					throw new ValidationException("该订单已经被派车单使用!");
			    }
			}//end zyb1
		...
		nc.vo.scm.pub.SCMEnv.out("弃审检测通过1"); /*-=notranslate=-*/
	}
	
	@SuppressWarnings("static-access")
	private Boolean isUnPass() throws nc.vo.pub.ValidationException{
		String vdef16  = getBillCardPanel().getHeadItem("vdef16")==null?"":getBillCardPanel().getHeadItem("vdef16").getValueObject().toString();
		String csaleid = getBillCardPanel().getHeadItem("csaleid")==null?"":getBillCardPanel().getHeadItem("csaleid").getValueObject().toString();
		
		if(vdef16!=null && vdef16.equals("其他销售")){
			//根据表头id查询计量单
			String sql = "select sourcebillid from gt_measure_h where nvl(dr,0)=0 and billtype='GJ08' and pk_busitype='其他销售' and sourcebillid='" +csaleid+ "'";
			String str = null;
			try {
			str = Proxy.getInstance().getItf().queryStrBySql(sql);
			} catch (BusinessException e) {
			e.printStackTrace();
			}
	 ...
	 * shiping 2014-10-20
	 * */
	private Boolean isPc() throws nc.vo.pub.ValidationException{
		String vdef17  = getBillCardPanel().getHeadItem("vdef17")==null?"":getBillCardPanel().getHeadItem("vdef17").getValueObject().toString();
		String csaleid = getBillCardPanel().getHeadItem("csaleid")==null?"":getBillCardPanel().getHeadItem("csaleid").getValueObject().toString();
		
		if(vdef17!=null &&  vdef17.contains("单品")){
			//根据表头id查询派车单
			String sql = "select BFPCD_PCZH from bfpcd where bfpcd_c3 is not null and BFPCD_SFBZ='1' and bfpcd_c8='" +csaleid+ "'";
			String str = null;
			try {
				str = Proxy.getInstance().getItf().queryStrBySql(sql);
			} catch (BusinessException e) {
			...
			if(str==null || str.length()==0)
				...
				return true;
		}else if(vdef17!=null &&  vdef17.contains("多品")){
			int rowcount = getBillCardPanel().getRowCount(); 
			for (int i = 0; i < rowcount; i++){
				Object corder_bid=getBillCardPanel().getBodyValueAt(i, "corder_bid");					
				//根据表体id查询派车单
				String sql = "select bfpcdmx_c3 from bfpcdmx where bfpcdmx_c3 is not null and BFPCD_SFBZ='1' and bfpcdmx_tdfl='" +corder_bid+ "'";
				String str = null;
				try {
					str = Proxy.getInstance().getItf().queryStrBySql(sql);
				} catch (BusinessException e) {
	...
	/**
	 * 查找所有已过皮派车单单号
	 * add by shipeng\sp\ship\施鹏
	 * @throws BusinessException 
	 * @time 2011-9-22
	 * */
	private String[] pccode() throws BusinessException{
		String csaleid = getBillCardPanel().getHeadItem("csaleid")==null?"":getBillCardPanel().getHeadItem("csaleid").getValueObject().toString();
		String[] vbillcodes=null;
		//查找所有已过皮派车单
		StringBuffer sql_gp=new StringBuffer();
		sql_gp.append("select distinct vbillcode")
			.append(" from gt_pcbill")
			.append(" where pk_pcbill in (select sourcebillid")
			.append("                   from gt_measure_h")
			.append("                   where nvl(dr, 0) = 0")
			.append("                and billtype = 'GJ08'")
			.append("                  and nvl(tareweight, 0) > 0")
			.append("                 and nvl(grossweight, 0) = 0")
			.append("              and sourcebillid in")
            .append("                  (select pk_pcbill")
            .append("                    from gt_pcbill")
            .append("                   where pk_deliveryplan in (select pk_deliveryplan from gt_deliveryplan_b where pk_saleorder='"+csaleid+"' and nvl(dr,0)=0)")
            .append("                      and nvl(dr, 0) = 0))")
            .append(" and pk_deliveryplan in (select pk_deliveryplan from gt_deliveryplan_b where pk_saleorder='"+csaleid+"' and nvl(dr,0)=0)")
            .append(" and nvl(dr, 0) = 0");			
		ArrayList<HashMap<String, String>> list_gp=Proxy.getInstance().getItf().queryArrayBySql(sql_gp.toString());
		if(list_gp!=null&&list_gp.size()>0){
			vbillcodes=new String[list_gp.size()];
			for(int i=0;i<list_gp.size();i++){
				HashMap<String, String> map=list_gp.get(i);
				vbillcodes[i]=map.get("vbillcode");
			}
			return vbillcodes;
		}
		return vbillcodes;
	 ...
	 * @time 2012-2-8
	 * */
	private boolean isPlanClosing() throws BusinessException{
		boolean isColsing=false;
		String csaleid = getBillCardPanel().getHeadItem("csaleid")==null?"":getBillCardPanel().getHeadItem("csaleid").getValueObject().toString();
		StringBuffer sql=new StringBuffer();
		sql.append("select * from ")
			.append(" (select vbillstatus,pk_deliveryplan from gt_deliveryplan where nvl(dr,0)=0 and vbillstatus<>9)a ")
			.append(" inner join (select pk_deliveryplan,pk_saleorder from gt_deliveryplan_b where pk_saleorder='"+csaleid+"' and nvl(dr,0)=0) b ")
			.append("on a.pk_deliveryplan=b.pk_deliveryplan");			
		ArrayList<HashMap<String, String>> list=Proxy.getInstance().getItf().queryArrayBySql(sql.toString());
	...
	protected boolean doException(SaleOrderVO vo, SaleOrderVO oldvo, Exception e) throws Exception {
		...
		if (e instanceof nc.vo.uap.pf.PFBusinessException) {
			// 错误信息中行号的所在位置
			String rowNo_s = "行号为"; /*-=notranslate=-*/
	...
	private void onBoSendInv() {
		try {
			...
			if (list.size() == 0) {
				throw new BusinessException(NCLangRes.getInstance().getStrByID("40060302", "UPP40060301-100062"));
			}
	...
	private void onBoSupplyDirectInv(ButtonObject bo) {
		try {
			...
			SaleorderBVO[] bvos = svo.getBodyVOs();
            //liyu1 直运安排增加状态判断
			if(!"2".equals(svo.getHeadVO().getFstatus().toString().trim()) )
			{
				throw new BusinessException(NCLangRes.getInstance().getStrByID("40060302", "UPP40060301-100063"));
			}
			for (int i = 0; i < bvos.length; i++) {
				UFBoolean bsendendflag = bvos[i].getBarrangedflag();
				if ((null==bsendendflag ||!bsendendflag.booleanValue())
			...
			if (list.size() == 0) {
				throw new BusinessException(NCLangRes.getInstance().getStrByID("40060302", "UPP40060301-100064"));
			}
			...
			if (getBillToBillUI(realVo).getMainCtrl() != null) {
				...
				showWarningMessage(NCLangRes.getInstance().getStrByID("40060302", "UPP40060301-100065")/*"补货直运涉及采购、内部交易、生产制造，请检查相关产品安装情况！"*/);
	...
	protected void onCopyBill() throws Exception {
		...
		timer.addExecutePhase(">>>>>>>>>销售订单[复制订单--复制条件检查]用时：>>>>>>>>>>>"); /*-=notranslate=-*/
		...
		timer.addExecutePhase(">>>>>>>>>销售订单[复制订单--复制前字段编辑性准备]用时：>>>>>>>>>>>"); /*-=notranslate=-*/
		...
		timer.addExecutePhase(">>>>>>>>>销售订单[复制订单--自由项准备]用时：>>>>>>>>>>>"); /*-=notranslate=-*/
		...
		timer.addExecutePhase(">>>>>>>>>销售订单[复制订单-- 非直运流程复制直运流程判断]用时：>>>>>>>>>>>"); /*-=notranslate=-*/
		...
		for (int i = 0; i < getBillCardPanel().getRowCount(); i++) {
			...
			//清空发货计划数add by shipeng/ship/sp/施鹏 at 2011-9-26
			getBillCardPanel().setBodyValueAt(null, i, "vdef20");//累计发货数量
			getBillCardPanel().setBodyValueAt(null, i, "vdef11");//累计下线数量
			getBillCardPanel().setBodyValueAt(null, i, "vdef7");//累计过磅块数
			//end by shipeng/ship/sp/施鹏 at 2011-9-26
		...
		timer.addExecutePhase(">>>>>>>>>销售订单[复制订单--循环设置表体行的可编辑性]用时：>>>>>>>>>>>"); /*-=notranslate=-*/
		
		setDefaultData(false);
		timer.addExecutePhase(">>>>>>>>>销售订单[复制订单--设置默认数据]用时：>>>>>>>>>>>"); /*-=notranslate=-*/
		...
		timer.addExecutePhase(">>>>>>>>>销售订单[复制订单-- 恢复模板项的初始编辑状态]用时：>>>>>>>>>>>"); /*-=notranslate=-*/
		...
		// 如果发现汇率有变化，则触发汇率事件，重新计算
		timer.addExecutePhase(">>>>>>>>>销售订单[复制订单--重新计算汇率]用时：>>>>>>>>>>>"); /*-=notranslate=-*/
		...
		getBillCardPanel().getHeadItem("nheadsummny").setValue(
				getBillCardPanel().getTotalValue("noriginalcursummny"));
		timer.addExecutePhase(">>>>>>>>>销售订单[复制订单--重新计算表头价税合计]用时：>>>>>>>>>>>"); /*-=notranslate=-*/
		...
		timer.addExecutePhase(">>>>>>>>>销售订单[复制订单--设置自由项]用时：>>>>>>>>>>>"); /*-=notranslate=-*/
		...
		timer.addExecutePhase(">>>>>>>>>销售订单[复制订单--尝试关联合同]用时：>>>>>>>>>>>"); /*-=notranslate=-*/
		
		// 销售订单复制后设置各字段可编辑性
		getBillCardPanel().setEditEnabledForCopy();

		//输出各个业务操作占用时间分布
		timer.showAllExecutePhase(">>>>>>>>>销售订单[复制订单]结束：>>>>>>>>>>>"); /*-=notranslate=-*/
	}
	...
	private void setCardButtonsState() {
		...
		// 没有单据加载的浏览状态
		if ((csaleid == null || csaleid.length() == 0) && (strState.equals("自由"))) { /*-=notranslate=-*/
			...
			//add by sk
			boImport.setEnabled(false);//导入
			boQTImport.setEnabled(false);//导入
			//取价 add  by shipeng at 2014-10-11
			boCalculate.setEnabled(false);
			//读卡
			boReadCard.setEnabled(false);
			//end by shipeng at 2014-10-11
			...
			//add by sk
			boImport.setEnabled(true);//导入
			boQTImport.setEnabled(true);//导入
			//end by sk 2011-10-24
			//取价 add  by shipeng at 2014-10-11
			boCalculate.setEnabled(true);
			//读卡
			boReadCard.setEnabled(true);
			//end by shipeng at 2014-10-11
			// yb add
...
//		if (!strState.equals("修订")) { /*-=notranslate=-*/
	...
	protected void onBom() {
		...
		if (row < 0) {
			showWarningMessage(NCLangRes.getInstance().getStrByID("40060302", "UPP40060301-100066")/*"请先选择表体行！"*/);
			return;
		}
		...
		if (ocustID == null || ocurrID == null || oinvID == null) {
			showWarningMessage(NCLangRes.getInstance().getStrByID("40060302", "UPP40060301-100067")/*"请先选择客户、币种、存货！"*/);
			return;
		}
		...
		if (nnumber != null && nnumber.toString().length() != 0) {
			...
			if (bomCurrID == null && (!strState.equals("新增") && !strState.equals("修改"))) { /*-=notranslate=-*/
				strBomState = "FREE";
	...
	protected void onBomSave() {
		...
		try {
			...
			showHintMessage(NCLangRes.getInstance().getStrByID("40060301", "UPP40060301-000136")/*
																								 ...
																								 * "开始保存数据...."
																								 */);
			SCMEnv.out("开始保存：" + System.currentTimeMillis()); /*-=notranslate=-*/
			if (strBomState.equals("ADD")) {
				if (isSaveStart) {
			...
			setBomButtonsState();
			// getBillTreeCardPanel().getCustTree().setEnabled(true);
			SCMEnv.out("保存结束" + System.currentTimeMillis()); /*-=notranslate=-*/
			showHintMessage(Message.getSaveSuccessMessage(s));
		} catch (ValidationException e) {
			showErrorMessage(e.getMessage());
		} catch (Exception e) {
			showErrorMessage(e.getMessage());
			SCMEnv.out("数据保存失败！"); /*-=notranslate=-*/
			// e.printStackTrace(System.out);
		}
	}
	...
	public void onButtonClicked(ButtonObject bo) {
		...
		if ("0001".equals(pk_corp)||"@@@@".equals(pk_corp)){
			showErrorMessage(NCLangRes.getInstance().getStrByID("40060301", "UPT40060301-000599")/*
					 * @res
					 * "销售订单不支持集团业务！"
					 */);
			return;
		...
		if (strShowState.equals("列表") /*-=notranslate=-*/
				...
				&& bo.getParent() != boAdd && bo.getParent() != boBusiType && bo != boImport && bo != boQTImport) {
				...
				if (bo.getTag() != null && bo.getTag().equals("APPROVE")) {
					...
					if (selrow != null && selrow.length > 0) {
						for (int i = 0; i < selrow.length; i++) {
							...
							if (iStatus != BillStatus.FREE && iStatus != BillStatus.AUDITING) {
								showErrorMessage(NCLangRes.getInstance().getStrByID("40060301",
...
/*							if (isExistWorkflow(selrow[i])) {
		...
		try {
			if (bo == boAddLine)
				...
				//中板导入
				onImport();
			else if (bo == boQTImport)
				//其它导入
				onQTImport();
			else if (bo == boCalculate)
				//取价 add by shipeng at 2014-10-11
				onBoCalculate();
			else if(bo==	boReadCard)//读卡
				onBoReadCard();
		    //end by shipeng at 2014-10-11
	 ...
	 * @time 2014-10-11
	 * */
	private void onBoCalculate() {
		//产品线（存货档案自动带出）、日期时间的有效区间，地区（客户基本档案的自定义项一），存货、自由项、质量等级、是否含税
		if ((getBillCardPanel().getHeadItem("pk_defdoc8") != null
				&& (getBillCardPanel().getHeadItem("pk_defdoc8").getValueObject()== null||"".equals(getBillCardPanel().getHeadItem("pk_defdoc8").getValueObject())))||
				(getBillCardPanel().getHeadItem("vdef8") != null
				&&(getBillCardPanel().getHeadItem("vdef8").getValueObject()== null||"".equals(getBillCardPanel().getHeadItem("vdef8").getValueObject())))) {
			showWarningMessage("价格地区不可为空");
			getBillCardPanel().getHeadItem("pk_defdoc8").setValue("");
			return;
		}
		Object pk_addprice=getBillCardPanel().getHeadItem("pk_defdoc8").getValueObject();
	    UFDate dbilldate = new UFDate(getBillCardPanel().getHeadItem("dbilldate").getValue());
	    String time=getClientEnvironment().getServerTime().getTime();
		Object vdef9=getBillCardPanel().getHeadItem("vdef9").getValueObject();
		if(vdef9==null||"".equals(vdef9)){
			showWarningMessage("请选择是否含税");
			return;
		}
		int rowcount = getBillCardPanel().getRowCount(); 
		for (int i = 0; i < rowcount; i++){
			Object cinvbasdocid=getBillCardPanel().getBodyValueAt(i, "cinvbasdocid");
			Object pk_defdoc18=getBillCardPanel().getBodyValueAt(i, "pk_defdoc18");
            if(cinvbasdocid==null||"".equals(cinvbasdocid)){
            	if(i==0){
            		showWarningMessage("表体没有存货信息");
            		return;
            	}
    			continue;
            }
			InvVO vo = (InvVO) getBillCardPanel().alInvs.get(i);
			Object vfree1=getBillCardPanel().getBodyValueAt(i, "vfree1");
			Object vfree2=getBillCardPanel().getBodyValueAt(i, "vfree2");
			Object vfree3=getBillCardPanel().getBodyValueAt(i, "vfree3");
			Object vfree4=getBillCardPanel().getBodyValueAt(i, "vfree4");
            StringBuffer sql=new StringBuffer();
            sql.append(" select  a.pk_corp dec,b.hsprice,b.vdef1,a.vbillcode,a.pk_pricepolicy  ") 
            .append("   from gt_pricepolicy a ") 
            .append("  inner join gt_pricepolicy_b b on a.pk_pricepolicy = b.pk_pricepolicy ") 
            .append("  where billtype = 'JY02' ") 
            .append("    and nvl(a.dr, 0) = 0 ") 
            .append("    and nvl(b.dr, 0) = 0 ") 
            .append("    and a.vbillstatus = 1 ") 
            .append("    and (a.pk_corp = '"+getCorpPrimaryKey()+"' or a.pk_corp = '0001') ") 
            .append("    and b.vdef12 = '"+cinvbasdocid+"' ") 
            .append("    and b.vdef11 = '"+pk_addprice+"' ") 
            .append("    and a.sdate <= '"+dbilldate+"' ") 
            .append("    and '"+dbilldate+"'<=a.edate ") 
            .append("    and a.stime<='"+time+"' ") 
            .append("    and '"+time+"'<=a.etime ") ;
    		if(!Toolkits.isEmpty(pk_defdoc18)){
//    			sql.append( " and b.pk_level ='"+pk_defdoc18+"'");
    			sql.append("    and b.pk_level  in (select ccheckstate_bid from qc_checkstate_b where ccheckstatename in (select ccheckstatename from qc_checkstate_b where ccheckstate_bid ='"+pk_defdoc18+"') and (pk_corp = '"+getCorpPrimaryKey()+"' or pk_corp = '0001'))");
    		}
    		else{
    			sql.append( " and b.pk_level is null ");
    		}
    		if(!Toolkits.isEmpty(vfree1)){
    			sql.append( " and ((decode(free1_2,0,free1_0)<="+vfree1)
    			.append( " and decode(free1_2,0,free1_1)>="+vfree1+" )or ")
    			.append( " (decode(free1_2,1,free1_0)<="+vfree1)
    			.append( " and decode(free1_2,1,free1_1)>"+vfree1+")or ")
    			.append( " (decode(free1_2,2,free1_0)<"+vfree1)
    			.append( " and decode(free1_2,2,free1_1)>="+vfree1+")or")
    			.append( " (decode(free1_2,3,free1_0)<"+vfree1)
    			.append( " or decode(free1_2,3,free1_1)>"+vfree1+"))");	
    		}
    		if(!Toolkits.isEmpty(vfree2)){
    			sql.append( " and ((decode(free2_2,0,free2_0)<="+vfree2)
    			.append( " and decode(free2_2,0,free2_1)>="+vfree2+" )or ")
    			.append( " (decode(free2_2,1,free2_0)<="+vfree2)
    			.append( " and decode(free2_2,1,free2_1)>"+vfree2+")or ")
    			.append( " (decode(free2_2,2,free2_0)<"+vfree2)
    			.append( " and decode(free2_2,2,free2_1)>="+vfree2+")or")
    			.append( " (decode(free2_2,3,free2_0)<"+vfree2)
    			.append( " or decode(free2_2,3,free2_1)>"+vfree2+"))");	
    		}
    		if(!Toolkits.isEmpty(vfree3)){
    			sql.append( " and ((decode(free3_2,0,free3_0)<="+vfree3)
    			.append( " and decode(free3_2,0,free3_1)>="+vfree3+" )or ")
    			.append( " (decode(free3_2,1,free3_0)<="+vfree3)
    			.append( " and decode(free3_2,1,free3_1)>"+vfree3+")or ")
    			.append( " (decode(free3_2,2,free3_0)<"+vfree3)
    			.append( " and decode(free3_2,2,free3_1)>="+vfree3+")or")
    			.append( " (decode(free3_2,3,free3_0)<"+vfree3)
    			.append( " or decode(free3_2,3,free3_1)>"+vfree3+"))");	
    		}
    		if(!Toolkits.isEmpty(vfree4)){
    			sql.append( " and ((decode(free4_2,0,free4_0)<="+vfree4)
    			.append( " and decode(free4_2,0,free4_1)>="+vfree4+" )or ")
    			.append( " (decode(free4_2,1,free4_0)<="+vfree4)
    			.append( " and decode(free4_2,1,free4_1)>"+vfree4+")or ")
    			.append( " (decode(free4_2,2,free4_0)<"+vfree4)
    			.append( " and decode(free4_2,2,free4_1)>="+vfree4+")or")
    			.append( " (decode(free4_2,3,free4_0)<"+vfree4)
    			.append( " or decode(free4_2,3,free4_1)>"+vfree4+"))");	
    		}
    		 ArrayList list;
			try {
				list = Proxy.getItf().queryArrayBySql(sql.toString());
	    		 if(list!=null&&list.size()>0){
	        		 String key="noriginalcurtaxprice"; 
	        		 Object price=null;
	        		 HashMap<String, String>map=(HashMap<String,String>)list.get(0);
//	        		 if(i==0){
//	        			 getBillCardPanel().getHeadItem("pk_defdoc13").setValue(map.get("pk_pricepolicy"));
//	        			 getBillCardPanel().getHeadItem("vdef13").setValue(map.get("vbillcode"));
//	        		 }
	        		 getBillCardPanel().setBodyValueAt(map.get("vbillcode"), i,"vdef13");
	        		 getBillCardPanel().setBodyValueAt(map.get("pk_pricepolicy"), i, "pk_defdoc13");
	        		 if(vdef9==null||"".equals(vdef9)||"N".equals(vdef9)||"否".equals(vdef9)){
	        			price=map.get("vdef1");
	        			getBillCardPanel().setBodyValueAt(price, i, key);
     					RelationsCal.calculate(i, price, getBillCardPanel(), SaleorderBVO.getCalculatePara(key, null, SA_02,
    							SO40), key, SaleorderBVO.getKeys(), SaleorderBVO.getField(), SaleorderBVO.class.getName(),
    							SaleorderHVO.class.getName(), null);  
	        		 }else{
	        			price=map.get("hsprice");
	        			getBillCardPanel().setBodyValueAt(price, i, key);
     					RelationsCal.calculate(i, price, getBillCardPanel(), SaleorderBVO.getCalculatePara(key, null, SA_02,
    							SO40), key, SaleorderBVO.getKeys(), SaleorderBVO.getField(), SaleorderBVO.class.getName(),
    							SaleorderHVO.class.getName(), null);  
	            		...
	            		showWarningMessage("没有取到价格，请确定是否维护好价格政策");
	            		return;
	    		 }
			} catch (BusinessException e) {
				// TODO Auto-generated catch block
	...
	private void onBoReadCard() {
		...
		Runnable checkrun = new Runnable(){
	        public void run(){
	                //线程对话框，系统运行提示框
	                BannerDialog dialog = new BannerDialog(SaleBillUI.this);
	                dialog.start();
	                ...
	                try{
	                	...
	                	if (epen != null){
	                		...
	                		if (res != null ) {
								...
								//车号
								getBillCardPanel().getHeadItem("vdef10").setValue(bills[0]);
								//派车单号
			        			getBillCardPanel().getHeadItem("vdef18").setValue(bills[1]);
							}
	                	} else {
...
//            res = fileChooser.showOpenDialog(getBillToBillUI());
//            if (res == 0) {
...
//        		               SaleorderBVO[] bvo = WriteToExcel.wbvo;
//        		       	    if(bvo!=null && bvo.length>0 ){
//        		       	  		StringBuffer info=new StringBuffer("");
//        		       	      	for(int i=0;i<bvo.length;i++){
//        		       	      		String err = bvo[i].getVdef1()==null?"":bvo[i].getVdef1().toString();
//        		       	      		if(err.trim().length()>0){
//        		       	      			info.append("第("+(i+4)+")行：("+err+")\n");
//        		       	      		}
//        		       	      	}
...
//        		       	    			ar[a]=a;
//        		       	    		}
//        		       	    		getBillCardPanel().getBillModel().delLine(ar);
//
//        		       	  			try {
//        		       	  				UFDouble zbhl=new UFDouble(0);
//        		       	  				UFDouble hl= Toolkits.getUFDouble(getBillCardPanel().getHeadItem("nexchangeotobrate").getValue());//汇率折本
//        		       	  				if(hl.doubleValue()==0){
//        		       	  					zbhl=Toolkits.getUFDouble(1);
//        		       	  				}else{
//        		       	  					zbhl=hl;
//        		       	  				}
//        		       	  				for(int j=0;j<bvo.length;j++){
//        		       	  					addLine();
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getCprolineid(), j, "cprolineid");//产品线
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getCinventoryid(), j, "cinventoryid");//存货
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getCrowno(), j, "crowno");//行号
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getNnumber(), j, "nnumber");//数量
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getNnumber(), j, "nquoteunitnum");//报价单位数量
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getNoriginalcurtaxprice(), j, "noriginalcurtaxprice");//含税单价
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getNoriginalcurtaxprice(), j, "norgqttaxnetprc");//报价单位含税单价
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getNoriginalcurtaxprice(), j, "noriginalcurtaxnetprice");//含税净价
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getNtaxrate(), j, "ntaxrate");//税率
//        		       	  					getBillCardPanel().setBodyValueAt(zbhl, j, "nexchangeotobrate");//折本汇率
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getNoriginalcurprice().multiply(zbhl), j, "nprice");//本币无税单价
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getNoriginalcurtaxprice().multiply(zbhl), j, "ntaxprice");//本币含税单价
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getNoriginalcurprice().multiply(zbhl), j, "nnetprice");//本币无税净价
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getNoriginalcurtaxprice().multiply(zbhl), j, "ntaxnetprice");//本币含税净价
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getNoriginalcurmny().multiply(zbhl), j, "nmny");//本币无税金额
//        		       	  					getBillCardPanel().setBodyValueAt((bvo[j].getNoriginalcursummny().sub(bvo[j].getNoriginalcurmny())).multiply(zbhl), j, "ntaxmny");//本币税额
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getNoriginalcursummny().multiply(zbhl), j, "nsummny");//本币价税合计
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getNoriginalcurprice(), j, "noriginalcurprice");//无税单价
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getNoriginalcurprice(), j, "norgqtnetprc");//报价单位无税单价
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getNoriginalcurprice(), j, "noriginalcurnetprice");//无税净价
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getNoriginalcurmny(), j, "noriginalcurmny");//无税金额
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getNoriginalcursummny(), j, "noriginalcursummny");//价税合计
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getNoriginalcursummny().sub(bvo[j].getNoriginalcurmny()), j, "noriginalcurtaxmny");//税额
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getCpackunitid(), j, "cpackunitid");//辅单位主键
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getNpacknumber(), j, "npacknumber");//辅数量
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getFrownote(), j, "frownote");//备注
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getVdef18(), j, "vdef18");//质量等级
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getPk_defdoc18(), j, "pk_defdoc18");//质量等级主键
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getVfree1(), j, "vfree1");//厚度下限
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getVfree2(), j, "vfree2");//宽度下限
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getVfree3(), j, "vfree3");//长度下限
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getVdef6(), j, "vdef6");//外形
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getPk_defdoc6(), j, "pk_defdoc6");//外形主键
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getVdef8(), j, "vdef8");//是否保性能
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getPk_defdoc8(), j, "pk_defdoc8");//是否保性能主键
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getVdef9(), j, "vdef9");//探伤级别
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getPk_defdoc9(), j, "pk_defdoc9");//探伤级别主键
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getVdef12(), j, "vdef12");//厚度上限
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getPk_defdoc12(), j, "pk_defdoc12");//厚度上限主键
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getVdef13(), j, "vdef13");//厚度条件
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getPk_defdoc13(), j, "pk_defdoc13");//厚度条件主键
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getVdef14(), j, "vdef14");//宽度上限
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getPk_defdoc14(), j, "pk_defdoc14");//宽度上限主键
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getVdef15(), j, "vdef15");//宽度条件
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getPk_defdoc15(), j, "pk_defdoc15");//宽度条件主键
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getVdef16(), j, "vdef16");//长度上限
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getPk_defdoc16(), j, "pk_defdoc16");//长度上限主键
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getVdef17(), j, "vdef17");//长度条件
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getPk_defdoc17(), j, "pk_defdoc17");//长度条件主键
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getVdef10(), j, "vdef10");//计量类型
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getPk_defdoc10(), j, "pk_defdoc10");//计量类型主键
//        		       	  				    getBillCardPanel().getBillModel("table").execLoadFormulaByRow(j);
//        		       	  					getBillCardPanel().setBodyValueAt((bvo[j].getNnumber()).div(bvo[j].getNpacknumber()), j, "scalefactor");//换算率
//        		       	  				}
//        		         				    getBillCardPanel().initFreeItem();//初始化自由项
//        		         				} catch (Exception e1) {
//        		       					e1.printStackTrace();
//        		       				}
//        		       	  		}
...
//                txtFile = fileChooser.getSelectedFile();
//                String filepath = txtFile.getAbsolutePath();
//                final WriteToExcel2 exceldata=new WriteToExcel2();
//                exceldata.creatFile(filepath);
//        		Runnable checkRun = new Runnable() {
//        			public void run() {
//        				BannerDialog dialog = new BannerDialog(	getParent());
...
//        				try {
//        		               exceldata.readData(0);
//        		               SaleorderBVO[] bvo = WriteToExcel2.wbvo;
//        		       	    if(bvo!=null && bvo.length>0 ){
//        		       	  		StringBuffer info=new StringBuffer("");
//        		       	      	for(int i=0;i<bvo.length;i++){
//        		       	      		String err = bvo[i].getVdef1()==null?"":bvo[i].getVdef1().toString();
//        		       	      		if(err.trim().length()>0){
//        		       	      			info.append("第("+(i+5)+")行：("+err+")\n");
//        		       	      		}
//        		       	      	}
//        		       	      	if(!info.toString().trim().equals("")){
//        		       	      		showErrorMessage(info.toString()+" 请核实!");
...
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getVdef18(), j, "vdef18");//质量等级
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getPk_defdoc18(), j, "pk_defdoc18");//质量等级主键
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getVfree1(), j, "vfree1");//厚度
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getVfree2(), j, "vfree2");//宽度
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getVfree3(), j, "vfree3");//长度
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getVfree4(), j, "vfree4");//直径
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getVdef10(), j, "vdef10");//计量类型
//        		       	  					getBillCardPanel().setBodyValueAt(bvo[j].getPk_defdoc10(), j, "pk_defdoc10");//计量类型主键
//        		       	  				    getBillCardPanel().getBillModel("table").execLoadFormulaByRow(j);
//        		       	  					getBillCardPanel().setBodyValueAt((bvo[j].getNnumber()).div(bvo[j].getNpacknumber()), j, "scalefactor");//换算率
...
//        }
	...
	public void onCancel() {
		...
		//add by zyb 清除缓存
//       getBillCardPanel().setHeadItem("vdef17", null);
		//add by ship at 2011-8-27
		String []vkey=new String[]{"vdef17","vdef18","vdef19"};
		for(String key:vkey){
			BillItem item = getBillCardPanel().getHeadItem(key);
			if (item!=null&&item.getDataType() == BillItem.USERDEF) 
	...
	protected void onCheck(SaleOrderVO saleorder) throws ValidationException, BusinessException {
		...
		//执行表头验证公式 
		getBillCardPanel().getBillData().execHeadTailValidateFormulas(null, null);
		//执行表体验证公式 
		getBillCardPanel().getBillModel().execValidateForumlas(null, null, null);
//		getBillCardPanel().getBillData().execValidateFormulas();
	...
	private void checkAllowDiffer(SaleorderBVO oldbodyVO, int i) throws ValidationException {
		...
		postiveNum = nnumber;
		//add by zyb 2012-02-11修订不判断是否超发货数
...
//			}
		...
		postiveNum = nnumber;
		//add by zyb 2012-02-11修订不判断是否超出库数
...
//		}
	...
	protected void onModification() {
		...
		//liyu1 修订时，能编辑仓库
		getBillCardPanel().getBodyItem("cbodywarehousename").setEnabled(true);
		for (int i = 0, len = getBillCardPanel().getRowCount(); i < len; i++) {
			getBillCardPanel().setCellEditable(i, "cbodywarehousename", true);
		}
...
/*		getBillCardPanel().getHeadItem("nexchangeotobrate").setEnabled(bedit_nexchangeotobrate_h);
	...
	protected void onCoRefPo(ButtonObject bo) throws Exception {
		...
		if (PfUIDataCache.getBillType("21") == null) {
			showErrorMessage(NCLangRes.getInstance().getStrByID("40060302", "UPP40060301-100068")/*"未安装或启用采购模块，无法进行购销协同。"*/);
			return;
		}
		if (strShowState.equals("列表")){ /*-=notranslate=-*/
        ...
        for(SaleOrderVO vo:saleOrderVOs){
          vo.getParentVO().setAttributeValue("ccalbodyid", vo.getChildrenVO()[0].getAttributeValue("cadvisecalbodyid"));
        }
		...
		try {
			...
			if (sSaleid != null) {
				...
				if (saleorder.getHeadVO().getAttributeValue("bcooptopo") != null
						&& ((UFBoolean) saleorder.getHeadVO().getAttributeValue("bcooptopo"))
								.booleanValue()) {
					showErrorMessage(NCLangRes.getInstance().getStrByID("40060302", "UPP40060301-100069")/*"该销售订单已协同生成采购订单。"*/);
					return;
				}
				if (saleorder.isCoopped()) {
					showErrorMessage(NCLangRes.getInstance().getStrByID("40060302", "UPP40060301-100069")/*"该销售订单已协同生成采购订单。"*/);
				...
				if (BillStatus.AUDIT != saleorder.getHeadVO().getFstatus()
					...
					showErrorMessage(NCLangRes.getInstance().getStrByID("40060302", "UPP40060301-100070")/*"只有审批后并且未有行关闭的销售订单才能进行此操作。"*/);
					return;
				...
				// updateCacheVO();
				showHintMessage(NCLangRes.getInstance().getStrByID("40060302", "UPP40060301-100069")/*"该销售订单已成功协同生成采购订单"*/);
			} else {
				showErrorMessage(NCLangRes.getInstance().getStrByID("40060302", "UPP40060301-100071")/*"请选择记录后再操作。"*/);
				return;
			}
		} catch (Exception e) {
			SCMEnv.error(e);
	...
	protected void onNew(ButtonObject bo) throws Exception {
		...
		try {
			if (nc.ui.pub.pf.PfUtilClient.makeFlag) {
				onNewBySelf();
				//ncm begin zhujha 201112241921521487_普阳钢铁_新增销售订单清空表头自定义项
				resumeBillDef();
				//ncm end zhujha 201112241921521487_普阳钢铁_新增销售订单清空表头自定义项
			} else {
				if (PfUtilClient.isCloseOK()) {
					binitOnNewByOther = true;
...
//		}
		...
		timer.addExecutePhase(">>>>>>>>>销售订单[参照上游单据确定]---默认数据处理：>>>>>>>>>>>"); /*-=notranslate=-*/

		//如果是来源于预订单,自动关联合同
		if ("38".equals(getSouceBillType()) && getBillCardPanel().bLinkCT){
			getBillCardPanel().setDefaultCtItem(0, getBillCardPanel().getBillTable().getRowCount(),true);
		}
		timer.addExecutePhase(">>>>>>>>>销售订单[参照上游单据确定]---自动关联合同用时：>>>>>>>>>>>"); /*-=notranslate=-*/
		
		// 自动关联买赠
		if (getBillCardPanel().bBindInvBuyLargess)
			getBillCardPanel().freshBodyLargess(2);
		timer.addExecutePhase(">>>>>>>>>销售订单[参照上游单据确定]---买赠捆绑询价用时：>>>>>>>>>>>"); /*-=notranslate=-*/
		
		getBillCardPanel().getBillModel().setNeedCalculate(true);
		
		//输出各个业务操作占用时间分布
		timer.showAllExecutePhase(">>>>>>>>>销售订单[参照上游单据确定]结束：>>>>>>>>>>>"); /*-=notranslate=-*/
	}
	...
	protected void onSplitPrint() {
		...
		try {
			...
			handleException(e, NCLangRes.getInstance().getStrByID("40060302", "UPP40060301-100072")/*"销售订单分单打印出现异常! "*/);
		}
	}
...
//			getPrintEntry().previewInDialog(true);
	...
	private AggregatedValueObject getPrintVO() {
		...
		//begin--浙江农资集团补丁：分单打印条件（发货仓库）  liuzwc 2010-05-19
		try {
			printHvo.getParentVO().setPrimaryKey((String)printHvo.getParentVO().getAttributeValue("csaleid"));
		} catch (BusinessException e) {
			handleException(e, NCLangRes.getInstance().getStrByID("40060302", "UPP40060301-100073")/*"销售订单分单打印设置printVO的PrimaryKey出错! "*/);
		}
		//end--
		return printHvo;
	...
	protected void onPrintListVos(boolean needPreview) throws Exception {
		...
		if (needPreview) {
			...
			for (int i = 0, loop = hvos.length; i < loop; i++) {
				// 处理打印日志
				//添加批打类型，以备printLogClient.check()时取值判断  liuzwc 2010-09-18 解决限制次数对于列表批打不管用
				ScmPrintlogVO logVO = hvos[i].getScmPrintlogVO();
				logVO.setAttributeValue("batchprinttype",Constants.EXPORT_TYPE_PRINT);
				printLogClient.setPrintInfo(logVO);
	...
	protected void onRefresh() {
		...
		for (int i = 0, len = voCondition.length; i < len; i++) {
			...
			if (
					!(
					((voCondition[i].getOperaCode().trim().equalsIgnoreCase("IS")
					&& voCondition[i].getValue().trim()
							.equalsIgnoreCase("NULL")) || 
							voCondition[i].getOperaCode().trim().equalsIgnoreCase("IS NULL"))
					
					|| (((voCondition[i].getValue() != null) && voCondition[i].getValue().trim()
							// "(select distinct power.resource_data_id"		
					.indexOf("(select ") >= 0))
							)
				) {
				if ("客户地区分类".equals(voCondition[i].getFieldName())) { /*-=notranslate=-*/
		...
		timer.addExecutePhase(">>>>>>>>>销售订单[查询--查询前准备条件]用时：>>>>>>>>>>>"); /*-=notranslate=-*/
		
		getBillListPanel().reLoadData();
		timer.addExecutePhase(">>>>>>>>>销售订单[查询--查询加载列表数据]用时：>>>>>>>>>>>"); /*-=notranslate=-*/
		...
		timer.addExecutePhase(">>>>>>>>>销售订单[查询--设置卡片信息]用时：>>>>>>>>>>>"); /*-=notranslate=-*/
		...
		b_query = true;
		setButtonsState();
		timer.addExecutePhase(">>>>>>>>>销售订单[查询--设置按钮状态]用时：>>>>>>>>>>>"); /*-=notranslate=-*/
		
		//输出各个业务操作占用时间分布
		timer.showAllExecutePhase(">>>>>>>>>销售订单[查询]结束：>>>>>>>>>>>"); /*-=notranslate=-*/
...
/*******2010-09-09 冯加滨 中铁物资：解决的问题：
在“销售订单——订单维护”节点的辅助功能“退回”，参照原销售订单进行退货时，生成新的销售订单的制单人依然是原销售订单的制单人，
而不是当前操作人员，影响到订单审批流的送审，客户希望这种情况生成退货的销售订单的制单人是当前登陆系统的操作人员。
*******/
			saleorderVO.getHeadVO().setCoperatorid(getClientEnvironment().getUser().getPrimaryKey());
...
/*			SCMRelationsCal.calculate(saleorderVO.getBodyVOs(), saleorderVO.getHeadVO(), 
			...
			s = System.currentTimeMillis();
			getBillCardPanel().setBillValueVO(saleorderVO);
			nc.vo.scm.pub.SCMEnv.out("数据设置[共用时" + (System.currentTimeMillis() - s) + "]"); /*-=notranslate=-*/
			...
			nc.vo.scm.pub.SCMEnv.out("执行公式[共用时" + (System.currentTimeMillis() - s1) + "]"); /*-=notranslate=-*/
...
/*			UIRefPane cadvisecalbody = (UIRefPane) getBillCardPanel().getBodyItem("cadvisecalbody")
			...
			nc.vo.scm.pub.SCMEnv.out("数据加载失败！"); /*-=notranslate=-*/
			nc.vo.scm.pub.SCMEnv.out(e.getMessage());
		}
	...
	protected void onBack() {
		...
		// 返回上一级界面
		if (strUISource.equals("列表")) /*-=notranslate=-*/
			onReturn();
		else
			onCard();
...
//		}
		...
		try {
			...
			nc.vo.scm.pub.SCMEnv.out("开始保存：" + System.currentTimeMillis()); /*-=notranslate=-*/
			...
			while (bContinue) {
				try {
					...
					if (strState.equals("修订")) { /*-=notranslate=-*/
						...
						//add by zyb 订单修订已发货数量大于订单数量不允许保存
						if(IsCanDelivery(saleorder)){
							showErrorMessage("累计发货数量大于订单数量，不允许保存!");
							return false;
						}//end by zyb 20120318
			...
			nc.vo.scm.pub.SCMEnv.out("保存结束：" + System.currentTimeMillis()); /*-=notranslate=-*/
	...
	/**
	 * 
	* 功能 订单修订是否可发货，可发货数量<0
	* 作者 朱永波 
	* 时间 2012-3-18 下午04:21:25 
	* 返回 boolean    返回类型
	 */
	public boolean IsCanDelivery(SaleOrderVO saleorder){
		boolean flag = false;		
		SaleorderBVO[] bvo = saleorder.getBodyVOs();
		for (SaleorderBVO saleorderBVO : bvo) {
			UFDouble ordernum = saleorderBVO.getNnumber()==null?new UFDouble(0):new UFDouble(saleorderBVO.getNnumber());
			UFDouble yfhnum =  saleorderBVO.getVdef20()==null?new UFDouble(0):new UFDouble(saleorderBVO.getVdef20());
			if(yfhnum.doubleValue()>ordernum.doubleValue()){
				flag = true;
				break;
			}
		}		
	...
	protected boolean onSaveExt() {
		...
		try {
			...
			if (ex instanceof SQLException){
				showWarningMessage(NCLangRes.getInstance().getStrByID("40060302", "UPP40060301-100074")/*"由于网络不稳定，可能已经生成此单据致，请检查后，进行确认!"*/);
				return bret;
			}
		...
		timer.addExecutePhase(">>>>>>>>>销售订单保存远程调用用时：>>>>>>>>>>>"); /*-=notranslate=-*/
		//输出各个业务操作占用时间分布
		timer.showAllExecutePhase(">>>>>>>>>销售订单保存远程调用结束：>>>>>>>>>>>"); /*-=notranslate=-*/
			...
			try {
				...
				if (ErrMsg != null && ErrMsg.startsWith("ERR")) {
					...
					if (ErrMsg != null && ErrMsg.trim().length() > 0) {
						...
						if (ErrMsg.indexOf("账期") > 0) /*-=notranslate=-*/
	...
	private void reLoadTS() {
		try {
			...
			nc.vo.scm.pub.SCMEnv.out("重新加载表头TS失败."); /*-=notranslate=-*/
			nc.vo.scm.pub.SCMEnv.out(e.getMessage());
		}
	}
	...
	private void setPanelBomByCurrency(String ccurrencytypeid) {
		try {
			...
			SCMEnv.out("根据币种设置小数位数失败!"); /*-=notranslate=-*/
			// e.printStackTrace();
		}
	}
	...
	private void setBtnsByBillState(int iState) {
		...
		if (strState.equals("修订") || strState.equals("修改") || strState.equals("新增") /*-=notranslate=-*/
				|| strState.equals("退货")) { /*-=notranslate=-*/
			boCoPushPo.setEnabled(false);
			return;
		}
	...
	private void initCTTypeVO() {
		...
		try {
			...
			SCMEnv.out("获取合同信息失败!"); /*-=notranslate=-*/
		}
	}
	...
	private void onApproveCheck(SaleOrderVO saleorder) throws nc.vo.pub.ValidationException {
		...
		SCMEnv.out("审核：" + getSouceBillType()); /*-=notranslate=-*/
		SCMEnv.out("审核：" + SaleBillType.SoDRP); /*-=notranslate=-*/
		...
		try {
					...
					if (oldbodyVO.getCreccalbodyid() == null
						...
						throw new ValidationException(NCLangRes.getInstance().getStrByID(
								"40060301", "UPP40060301-000202")/*
																	 ...
																	 * "下列字段不能为空：发货公司非本销售公司行的收货库存组织"
																	 */
								+ "\n" + NCLangRes.getInstance().getStrByID("40060302", "UPP40060301-100075")/*"行号："*/ + oldbodyVO.getCrowno());
					}
					if (oldbodyVO.getBdericttrans() == null
							|| !oldbodyVO.getBdericttrans().booleanValue()) {
						if (oldbodyVO.getCrecwareid() == null
			...
			//
			SCMEnv.out("审核检测通过1"); /*-=notranslate=-*/
		} catch (Throwable e) {
			throw new nc.vo.pub.ValidationException(NCLangRes.getInstance().getStrByID("40060301",
					"UPP40060301-000247")/*
	...
	protected void onCachPay() {
		...
		try {
			...
			/**v5.7 要求全单红蓝一致，才可做订单收款*/
			boolean ifMoreZero = false;
			boolean ifLessZero = false;
			for (SaleorderBVO bvo : bodyVOs){
				UFDouble summny = bvo.getNoriginalcursummny();
				if (summny.compareTo(UFDouble.ZERO_DBL)>0)
					ifMoreZero = true;
				else if (summny.compareTo(UFDouble.ZERO_DBL)<0)
					ifLessZero = true;
			}
			if (ifMoreZero && ifLessZero) {
				showErrorMessage(NCLangRes.getInstance().getStrByID(
						"40060301", "UPP40060301-000573")/*
														 * @res "要求全单红蓝一致，才可做订单收款"
														 */);
				return;
			}
			/**v5.7 要求全单红蓝一致，才可做订单收款*/
			...
			timer.addExecutePhase(">>>>>>>>>销售订单[订单收款]销售订单数据准备用时：>>>>>>>>>>>"); /*-=notranslate=-*/
			
			// vo交换：3FtoD2
			DJZBVO djzbvo = (DJZBVO) PfChangeBO_Client.pfChangeBillToBill(srvo, "3F", dest[1]);
			timer.addExecutePhase(">>>>>>>>>销售订单[订单收款]vo交换用时：>>>>>>>>>>>"); /*-=notranslate=-*/
			...
			timer.addExecutePhase(">>>>>>>>>销售订单[订单收款]打开收款单节点用时：>>>>>>>>>>>"); /*-=notranslate=-*/
			
			timer.addExecutePhase(">>>>>>>>>销售订单[订单收款]远程调用用时：>>>>>>>>>>>"); /*-=notranslate=-*/
			//输出各个业务操作占用时间分布
			timer.showAllExecutePhase(">>>>>>>>>销售订单[订单收款]远程调用结束：>>>>>>>>>>>"); /*-=notranslate=-*/
	...
	public SaleReceiveVO getSaleReceiveVO(SaleOrderVO ordVO) {
		...
		try {
			...
			nc.vo.scm.pub.SCMEnv.out("calcMny:计算金额失败"); /*-=notranslate=-*/
		}
	...
	protected void onOrderBalance() {
		try {
			...
			if (!nc.ui.so.pub.BillTools.ifNodeCodeUse("40060308")) {
				showWarningMessage(NCLangRes.getInstance().getStrByID("40060302", "UPP40060301-100076")/*"订单核销节点未启用，该按钮不可用！"*/);
				return;
			}
			...
			for (int i = 0, len = btns.length; i < len; i++) {
				if (btns[i].getCode().equals("列表显示") || btns[i].getCode().equals("浏览") /*-=notranslate=-*/
						|| btns[i].getCode().equals("打印管理")) { /*-=notranslate=-*/
	...
	public boolean onClosing() {
		...
		if (strState.equals("修改") || strState.equals("修订") || strState.equals("新增")) { /*-=notranslate=-*/
	...
	public void openSaleOrderByID(String billid){
		...
		timer.addExecutePhase(">>>>>>>>>销售订单[联查进入销售订单--列表转卡片]用时：>>>>>>>>>>>"); /*-=notranslate=-*/
		
        id = billid;
		loadCardDataByID(billid, false);
		timer.addExecutePhase(">>>>>>>>>销售订单[联查进入销售订单--卡片置数]用时：>>>>>>>>>>>"); /*-=notranslate=-*/
		
		setCardButtonsState();
		timer.addExecutePhase(">>>>>>>>>销售订单[联查进入销售订单--设置卡片按钮状态]用时：>>>>>>>>>>>"); /*-=notranslate=-*/
		...
		timer.addExecutePhase(">>>>>>>>>销售订单[联查进入销售订单--设置列表数据]用时：>>>>>>>>>>>"); /*-=notranslate=-*/
		
		//输出各个业务操作占用时间分布
		timer.showAllExecutePhase(">>>>>>>>>销售订单[联查进入销售订单]结束：>>>>>>>>>>>"); /*-=notranslate=-*/
	}
